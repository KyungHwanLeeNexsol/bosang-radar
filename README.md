...
> **2026-09-17 제품 방향 전환**: 보상레이더는 사고·질병 당사자 **본인**이 검색 한 줄로 놓치고 있는 보험 담보를 진단받고 손해사정사 상담으로 연결되는 **B2C 보상 진단 퍼널**로 방향을 전환했습니다(`design/MIGRATION-PLAN.md` 기준). **다만 이 저장소에 실제로 구현·배포되어 있는 코드는 아직 이전 B2B 방향입니다** — 새 B2C 화면은 디자인만 확정되었을 뿐 코드는 없습니다. 아래 설명과 기술 스택·실행 방법은 모두 현재 구현되어 있는 B2B 버전 기준입니다.

보상레이더(B2B 버전)는 보험설계사와 손해사정사가 비식별 보험 사건 정보를 입력하면, 상해후유장해·질병후유장해 관점에서 추가로 검토할 담보·근거자료·반대 논리·추가 필요자료를 조사해 주는 AI Research Assistant입니다. 사전에 지정한 실무자 10명 내외를 대상으로 하는 비공개 파일럿이며, 회원가입은 없고 운영자가 등록한 테스터 계정만 초대 전용(allowlist)으로 로그인합니다.

새 B2C 방향의 제품 정의·화면 구성·두 방향의 공존 관계를 포함한 자세한 제품 설명은 [`.moai/project/product.md`](.moai/project/product.md)를 참고하세요.
