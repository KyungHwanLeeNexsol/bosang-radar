...
export async function challenge(
  ...
  for (const item of result.data.challenges) {
    ...
    // (3.5) post-run fix: counterArgument가 빈 문자열이면 그 항목만 개별
    // 폐기한다 — Zod 스키마는 counterArgument: z.string()으로만 검증하고
    // .min(1)을 두지 않으므로(배치 부분 실패 설계 유지, 상단 주석 참고),
    // 파싱 성공 이후 항목별 업무 규칙으로 비어있지 않음을 재확인한다. 빈
    // evidence 배열([]/[])을 그대로 허용하는 기존 계약(위 주석)과는 별개의
    // 검사다 — evidence 유무와 무관하게 counterArgument 텍스트 자체는
    // 비어있으면 안 된다.
    if (item.counterArgument.length < 1) {
      continue;
    }
