...
- [x] App Shell 사이드바: 5개 nav 항목(3개 실제 + 2개 비활성)과 사용자 블록(로그인 상태/로딩 상태 각각)이 Pencil 스펙과 육안으로 대응하는가 — `after-round4/exception-not-found-1440.png`(짧은 페이지, 사이드바 전체 + 사용자 블록 "e2e-tester-a" 하단에 정상 렌더링 확인) + `after-round4/mobile-drawer-open-390.png`(모바일 드로어에서도 사용자 블록 정상 렌더링) 직접 열람으로 확인. **잔여 관찰(체크를 막지 않음, 아래 각주 참조)**: `/cases/new`처럼 폼이 뷰포트보다 긴 데스크톱(≥1024px) 화면에서는 사용자 블록을 보려면 페이지 하단까지 스크롤해야 한다 — 2026-09-08 직접 조사(Playwright bounding-box 측정, `aside` 높이 1268px vs 뷰포트 768px)로 확인. spec.md/acceptance.md 어디에도 사이드바를 뷰포트에 고정(sticky/fixed)해야 한다는 요구사항이 없어 이 SPEC의 AC 위반은 아니며, 사용자 블록 자체의 스타일(이니셜 배지+이름+구분선)은 짧은 페이지에서 확인한 대로 Pencil 패턴과 정확히 일치한다. 후속 SPEC 후보로 남긴다(아래 §4 이후 기록 참조)
...
- [x] 실재하는 예외 화면 3종(전역 404 / 사건-없음·미소유 통합 / 기존 일시 런타임 오류): 각각의 아이콘·문구·에러코드 패턴이 Pencil 프레임 `11`의 대응 변형과 육안으로 정합하며, App Shell 유무(전역 404만 미적용)가 의도대로 렌더링되는가 — `after-round4/global-not-found-1440.png`(App Shell 미적용, `?` 아이콘 + "페이지를 찾을 수 없습니다" + `404 · ERR_NOT_FOUND`) + `after-round4/exception-not-found-1440.png`(App Shell 적용, `?` 아이콘 + "사건을 찾을 수 없습니다" + `사건 관리 · ERR_CASE_NOT_FOUND` + CTA 2개), 2026-09-08 직접 열람으로 확인. 기존 런타임 오류(`error.tsx`)는 REQ-015 범위에서 최소 검증만 수행하며 무변경(§ M7 참조) — 별도 신규 시각 확인 불필요
...
>
> **2026-09-08 재검토 (2차 correction pass)**: 외부 독립 재검토가 1차 correction pass의 결함을 지적했다 — 위 §3 시각 스모크 체크리스트의 6개 항목 자체는 여전히 `[ ]`로 남아 있었는데, 이 §4의 "§3 전부 확인" 항목만 `[x]`로 체크해 두 문서 상태가 모순됐다. 1차 pass에서 §4 재분류에 집중하느라 §3 원본 체크박스 자체를 실제로 대조하지 않고 넘어간 것이 원인이다. 이번에 §3의 6개 항목을 production 서버 기준으로 직접 재확인(스크린샷 직접 열람 + 1건은 신규 Playwright 조사)한 뒤 각 항목에 근거를 남기고 체크했다 — 상세: 위 §3 각 항목의 인라인 근거 참조. 조사 중 발견한 사이드바 사용자 블록 스크롤-가시성 관찰(§3 두 번째 항목 각주)은 이 SPEC의 어떤 REQ/AC도 위반하지 않아 DoD를 막지 않으며, 후속 SPEC 후보로 `progress.md`에 추가 기록했다.
