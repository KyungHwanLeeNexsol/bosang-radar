...
> **2026-09-08 재검토 (2차 correction pass)**: 외부 재검토가 이 §3의 6개 항목이 전부 `[ ]`로 남아 있는데도 §4 DoD는 "§3 전부 확인"으로 체크돼 있던 모순을 지적했다 — 1차 correction pass(2026-09-08)에서 §4만 고치고 §3 자체는 실제로 대조하지 않은 채 넘어간 결함이었다. 이번에 로컬 production 서버(`pnpm build && pnpm start`)로 6개 화면을 직접 재확인하고, 아래 각 항목에 실제로 확인한 근거(스크린샷 경로 또는 이번 세션 직접 관찰)를 남긴 뒤에만 체크했다.

- [x] 로그인 화면: design.md §4(로그인) 구조와 브랜드 패널·필드·비밀번호 토글·비활성 푸터 링크가 육안으로 대응하는가 — `docs/evidence/SPEC-UI-MIGRATION-001/after-round5/login-{1440,1024,390}.png`(production 재캡처, 2026-09-08 직접 열람) + Round 4/5 Gap Matrix(`progress.md`) 실측으로 확인
- [x] App Shell 사이드바: 5개 nav 항목(3개 실제 + 2개 비활성)과 사용자 블록(로그인 상태/로딩 상태 각각)이 Pencil 스펙과 육안으로 대응하는가 — **2026-09-08 3차 correction pass에서 실제 코드 수정 + 재대조로 확정(사용자 승인)**. 이전 기록은 `after-round4/exception-not-found-1440.png`(짧은 페이지) + `mobile-drawer-open-390.png`(모바일 드로어)만 근거로 삼아, 폼이 뷰포트보다 긴 데스크톱(≥1024px) 화면(예: `/cases/new`)에서 사용자 블록이 스크롤 없이는 보이지 않는 실제 UI 차이를 "REQ/AC에 명시 없음"이라는 이유로 체크를 막지 않는 잔여 관찰로만 남겼다 — 이는 "REQ/AC에 명시 없음"과 "Pencil 디자인에 부합함"을 같은 의미로 취급한 결함이었다(외부 재검토 지적). `design/exports/04-App-Shell.png` 자체가 "좌측 사이드바 232px + 상단바 62px **고정**"이라고 명시적으로 서술하고 있어, 사용자 블록이 스크롤 없이 항상 보여야 하는 것이 실제로 Pencil 스펙이었음을 재확인했다. 사용자에게 두 선택지(sticky/fixed 수정 vs 승인된 편차+후속 SPEC)를 AskUserQuestion으로 제시했고, "지금 sticky/fixed로 수정(권장)"이 선택됐다(2026-09-08). `app/cases/app-shell-chrome.tsx`의 `<aside>`를 `lg:static` → `lg:sticky lg:top-0 lg:h-screen`으로 수정, 신규 회귀 테스트 `e2e/sidebar-sticky.spec.ts`(1024px/1440px 각각 aside 높이 ≤ 뷰포트, 사용자 블록 y+height ≤ 뷰포트, 가로 오버플로 없음 assert) 1/1 PASS, 기존 `app-shell-chrome.test.tsx`(드로어 접근성 계약) 18/18 PASS 무회귀, `e2e/mobile-drawer-focus.spec.ts` 1/1 PASS 무회귀. 재캡처 증빙: `after-round5/case-input-sticky-sidebar-{1024,1440}.png`(2026-09-08 직접 열람 — 두 뷰포트 모두 사용자 블록 "e2e-tester-a"가 스크롤 없이 사이드바 하단에 보임, 본문/우측 레일 겹침·가로 오버플로 없음). **2026-09-08 4차 재검증**: 외부 재검토가 커밋된 1024px 이미지에서 사용자 블록이 안 보인다고 재차 지적해, `.next` 캐시를 완전히 삭제하고 fresh `pnpm build`로 재조사했다 — computed style(`position: sticky`, `top: 0px`, `height` = 뷰포트와 동일) + bounding box(사용자 블록 하단 1024px→744.375px≤768px, 1440px→876.375px≤900px) + `window.scrollY === 0` + 가로 오버플로 없음을 실측한 결과 **코드 결함은 발견되지 않았다**. 실제로 존재했던 gap은 캡처 테스트가 스크린샷 전에 assertion을 수행하지 않았던 것뿐이며, 공통 helper `e2e/sidebar-assertions.ts`(`assertStickySidebarUserBlockVisible`)로 `sidebar-sticky.spec.ts`와 `capture-evidence-round5.spec.ts` 양쪽에 동일한 캡처-전-assertion을 적용해 이 gap을 막았다(§ "Round 5 — 1024px 사이드바 증빙 재검증 (4차)" 참조).
- [x] App Topbar: `/cases/new`(브레드크럼 "작업 공간 / 사건 입력" · 타이틀 "신규 사건 리서치 요청")와 `/cases/[caseId]`(브레드크럼 "작업 공간 / 리서치 리포트" · 타이틀 "리서치 리포트", `#expert-feedback` 프래그먼트 유무와 무관하게 고정)가 spec.md REQ-006의 정확한 매핑대로 렌더링되는가 — `after-round4/case-input-empty-recent-1024.png`("작업 공간 / 사건 입력" + "신규 사건 리서치 요청" 확인) + `after-round5/report-verified-claim-1440.png`("작업 공간 / 리서치 리포트" + "리서치 리포트" 확인), 2026-09-08 직접 열람
- [x] 실재하는 예외 화면 3종(전역 404 / 사건-없음·미소유 통합 / 기존 일시 런타임 오류): 각각의 아이콘·문구·에러코드 패턴이 Pencil 프레임 `11`의 대응 변형과 육안으로 정합하며, App Shell 유무(전역 404만 미적용)가 의도대로 렌더링되는가 — `after-round4/global-not-found-1440.png`(App Shell 미적용, `?` 아이콘 + "페이지를 찾을 수 없습니다" + `404 · ERR_NOT_FOUND`) + `after-round4/exception-not-found-1440.png`(App Shell 적용, `?` 아이콘 + "사건을 찾을 수 없습니다" + `사건 관리 · ERR_CASE_NOT_FOUND` + CTA 2개), 2026-09-08 직접 열람으로 확인. **런타임 오류(`error.tsx`) — 2026-09-08 3차 correction pass에서 실제 재현·캡처로 교체**: 이전 기록("코드 무변경이라 별도 시각 확인 불필요")은 코드 무변경과 시각 확인을 혼동한 결함이었다 — 코드가 안 바뀐 것과 실제로 그 화면이 렌더링되는 걸 본 것은 다른 사실이다. 정상 플로우로는 이 오류 경계가 트리거되지 않으므로, `e2e/capture-evidence-round5.spec.ts`("런타임 오류 경계 — 실제 예외 트리거 후 error.tsx 렌더링 캡처")가 DB의 `reports.content.verifiedClaims`를 결정론적으로 제거해 `page.tsx`의 실제 코드 경로(`report.verifiedClaims.flatMap(...)`, report는 truthy이나 verifiedClaims 없어 TypeError)가 Next.js의 진짜 오류 경계로 이어지는지 프로덕션 서버에서 관찰했다 — 캡처 직전 "문제가 발생했습니다" 텍스트와 `case-error-retry` 버튼의 실제 가시성을 assert로 확인한 뒤 스크린샷을 저장했고, 재시도 버튼 클릭 후에도 크래시 없이 동일 화면이 안정적으로 재렌더링됨을 확인했다(§E.2 M7/설계 문서가 명시한 대로 에러코드 텍스트 없음 · App Shell 적용됨과 정확히 일치). 결과: `after-round5/runtime-error-{1440,1024}.png`(신규, production 재캡처), 잘림/겹침/가로 오버플로 없음 확인
- [x] 태블릿(1024px)/모바일(390px): 사이드바 유지+우측 레일 이동, 오프캔버스 드로어+스크림(열기/닫기/ESC/스크림클릭 각각)이 육안으로 확인되는가 — `after-round4/case-input-empty-recent-1024.png`(태블릿, 사이드바 고정폭 유지) + `after-round4/report-verified-claim-1024.png`(태블릿) + `after-round4/mobile-drawer-{open,closed}-390.png`(모바일 햄버거 닫힘/드로어+스크림 열림), 2026-09-08 직접 열람. 열기/닫기/ESC/스크림클릭 각각의 실제 동작은 AC-017a~d 자동화 테스트(`app-shell-chrome.test.tsx`)로 별도 검증됨(§E.2 M8 참조) — 이 체크리스트 항목은 정적 시각 상태(열림/닫힘 각 1장) 대조를 의미하며 그 범위에서 확인 완료
- [x] 6개 화면(로그인/사건 입력/리포트/피드백/예외 화면/App Shell 전반) 모두 전문 손해사정사에게 보여줄 때 "미완성"으로 보이지 않는 B2B 완성도 수준인가 — 위 5개 항목의 근거 스크린샷 + `after-round5/expert-feedback-initial-1440.png`(카드형 UI, 사건 메타 스트립, 우측 진행률/제출상태 레일)를 2026-09-08 직접 열람해 종합 판단. 일관된 다크 사이드바/퍼플 액센트/타이포그래피, 자리표시자성 텍스트나 미완성으로 보이는 요소 없음을 확인

## §4. Definition of Done

> **2026-09-08 재분류 (correction pass, 1차)**: 아래 6개 항목은 Round 5까지의 누적 실측(§E.2 M1~M8, Round 3/4/5 각 Gap Matrix, Round 5 최종 검증 §)을 근거로 체크한다. 3건의 잔여 논쟁 항목(로그인 우측 폼 간격/모바일 리포트·피드백 IA 분리/Pencil 모바일 프레임 동일폭 비교)의 재분류 근거는 `progress.md` "Round 5 — 3건 재분류 (correction pass)" 섹션 참조 — 요약: 항목1은 구현 완료, 항목2는 애초에 이 acceptance.md의 어떤 AC에도 대응하지 않는 항목(사용자 승인된 편차 + 후속 SPEC 후보), 항목3은 Pencil 원본(`13-Mobile-390.png`)이 3-패널 합성이라 방법론적으로 성립하지 않는 비교였고 개별 실화면 캡처로 대체 검증됨(낮은 잔여 위험으로 기록, DoD를 막지 않음).
>
> **2026-09-08 재검토 (2차 correction pass)**: 외부 독립 재검토가 1차 correction pass의 결함을 지적했다 — 위 §3 시각 스모크 체크리스트의 6개 항목 자체는 여전히 `[ ]`로 남아 있었는데, 이 §4의 "§3 전부 확인" 항목만 `[x]`로 체크해 두 문서 상태가 모순됐다. 1차 pass에서 §4 재분류에 집중하느라 §3 원본 체크박스 자체를 실제로 대조하지 않고 넘어간 것이 원인이다. 이번에 §3의 6개 항목을 production 서버 기준으로 직접 재확인(스크린샷 직접 열람 + 1건은 신규 Playwright 조사)한 뒤 각 항목에 근거를 남기고 체크했다 — 상세: 위 §3 각 항목의 인라인 근거 참조. 조사 중 발견한 사이드바 사용자 블록 스크롤-가시성 관찰(§3 두 번째 항목 각주)은 이 SPEC의 어떤 REQ/AC도 위반하지 않아 DoD를 막지 않는다고 2차 pass는 판단했으나, 이 판단은 3차 pass에서 다시 정정됐다(아래).
>
> **2026-09-08 3차 재검토(외부 재검토 결함 2건 정정)**: 외부 독립 재검토가 2차 pass의 판단 오류 2건을 지적했다. **(a) 런타임 오류 화면**: §3의 "실재하는 예외 화면 3종" 중 `error.tsx` 항목이 "코드 무변경이라 별도 시각 확인 불필요"라는 이유로 체크됐었다 — 코드 무변경과 실제 화면 렌더링 확인은 서로 다른 사실이며, 하나가 다른 하나를 대신할 수 없다. `e2e/capture-evidence-round5.spec.ts`에 결정론적 DB 손상(`reports.content.verifiedClaims` 제거)으로 실제 오류 경계를 트리거하는 테스트를 추가해 production 서버에서 진짜로 재현·캡처했다(위 §3 해당 항목 참조). **(b) 사이드바 사용자 블록**: "REQ/AC에 명시되지 않음"을 "Pencil 디자인에 부합함"과 동일시한 것이 결함이었다 — 실제로 `design/exports/04-App-Shell.png`는 사이드바가 "고정"이어야 한다고 명시하고 있었다. 사용자에게 AskUserQuestion으로 결정 게이트를 실행했고, "지금 sticky/fixed로 수정(권장)"이 선택되어 `app-shell-chrome.tsx`를 실제로 수정했다(위 §3 해당 항목 참조 — 코드 diff, 신규 회귀 테스트, 재캡처 증빙 전부 포함). 이번 3차 pass는 문서만이 아니라 **실제 코드 1개 파일**(`app/cases/app-shell-chrome.tsx`)을 수정했다는 점에서 1·2차 pass(문서 전용)와 다르다 — `git diff`로 이 파일 외 어떤 PRESERVE 대상도 건드리지 않았음을 확인했다(AC-019).
