...
2. **세션 조회 전략 결정 — 사이드바 실사용자명(클라이언트 분리, 무변경) + `/cases/new` 최근 리서치의 서버측 세션 확인(신규, 페이지 레벨) — 빌드-안전성 직결, `next.config.ts`/`lib/auth/session.ts` 직접 조사로 확정됨**: `next.config.ts`에 Partial Prerendering/Cache Components(`experimental.ppr`/`cacheComponents`)이 설정되어 있지 않음을 직접 확인했다(이전 캐싱 모델 적용). 이 상태에서는 서버 컴포넌트 어디서든(Suspense로 감싸도) Request-time API(`headers()` 등을 호출하는 `getCurrentSession()`)를 호출하면 그 라우트 전체가 정적 생성에서 동적 렌더링으로 전환된다 — `app/cases/layout.tsx`가 직접 세션을 조회하면 `/cases/new`의 정적 생성이 깨진다는 기존 코드 주석의 경고가 그대로 재현된다. 따라서 사이드바 사용자 블록은 별도 클라이언트 컴포넌트로 분리하고, 이미 프로젝트에 설치·사용 중인 `authClient`(`better-auth/react`, `lib/auth/client.ts`)의 세션 훅으로만 브라우저 하이드레이션 이후 조회한다 — 클라이언트 컴포넌트의 실행은 `next build`의 서버 렌더링 단계와 무관하므로 어떤 라우트의 정적 생성도 깨지지 않는다. 대안 1(레이아웃에서 직접 조회)은 정적 생성 파괴로 기각. 대안 2(Suspense로 감싼 서버 컴포넌트 격리)는 PPR/Cache Components 미설정 상태에서 효과가 없어(§C M2 참고) 기각. **이 결정은 사이드바에 한정된다 — 여기까지는 v0.2.0과 무변경이다.**
   **v0.3.0 신규 결정 — REQ-013 "최근 리서치"의 세션 조회는 정반대 요구(빌드-시점 owner-scoped DB 조회 불가 vs `/cases/new` 정적 생성 유지)를 안고 있어 사이드바와 동일한 "클라이언트 분리" 해법을 적용할 수 없다(클라이언트 컴포넌트는 DB에 직접 접근할 수 없다). 해결: `/cases/new`의 서버 페이지 컴포넌트(`NewCasePage`, `app/cases/new/page.tsx`)가 `app/cases/[caseId]/page.tsx`와 동일한 `getCurrentSession()` 패턴을 스스로 호출해 `ownerUserId`를 확보하고, §B 결정 6의 신규 조회 함수에 전달한다 — 신규 API 라우트 없음, `app/cases/layout.tsx`는 여전히 세션을 직접 호출하지 않는다(위 사이드바 결정 무변경). 이 결과 `NewCasePage`가 Request-time API(`headers()`, `getCurrentSession()` 경유)를 호출하므로, Next.js 16.3.2의 기본 라우트 세그먼트 설정(`dynamic: "auto"`)에 따라 `/cases/new`는 **자동으로** 동적(요청 시점) 렌더링으로 전환된다 — `export const dynamic = "force-dynamic"` 같은 명시적 선언은 필요 없다(research.md §5c 신규, Next.js 공식 문서 Route Segment Config `dynamic: "auto"`: "The default option to cache as much as possible without preventing any components from opting into dynamic behavior."). 이는 `app/cases/[caseId]/page.tsx`가 이미 동일한 방식으로 동적 렌더링되고 있는 것과 정확히 같은 메커니즘이며, 이미 이 코드베이스에서 안전이 입증된 패턴을 `/cases/new`로 확장하는 것이다. **결론(REQ-005a 폐기)**: "`/cases/new`는 반드시 정적 생성을 유지해야 한다"는 이전 요구사항(REQ-005a)은 REQ-013과 직접 모순되므로 폐기하고, 대신 빌드-안전성 요구사항(사용자별 데이터 빌드-시점 미평가, 빌드 자체가 실제 DB 연결을 요구하지 않음, 세션/DB 조회는 요청 시점에만 발생, owner-scope 서버측 강제)으로 대체한다(spec.md REQ-013 참고). `proxy.ts`의 쿠키 존재 확인은 이 신뢰 경계가 아니다 — `getCaseForOwner`와 동일하게 진짜 서버측 세션 검증 + owner 필터링을 이 조회 자신이 수행한다. 대안(신규 공개 API 라우트를 만들어 클라이언트에서 fetch)은 REQ-013의 "신규 공개 API 라우트 없음" 원칙과 충돌해 기각. 대안(레이아웃 레벨에서 조회해 props로 전달)은 사이드바 결정과 동일한 이유로 레이아웃의 관심사 분리를 깨뜨려 기각.
...
6. **"최근 리서치" 신규 read-only 조회 함수 도입 결정**: 신규 스키마 없이 기존 `cases` 테이블만 조회하는 소형 함수를 `lib/cases/`에 추가한다 — `getCaseForOwner`와 동일한 owner-scope 신뢰 경계를 재사용한다. `cases` 테이블에 `title` 컬럼이 없음을 확인했으므로(research.md §9), 제목/보조정보는 `input.diagnosisName`/`input.disabilityBodyPart`에서 파생하고, 필드 누락 시 정의된 폴백 문자열을 사용한다. 이 함수가 필요로 하는 `ownerUserId`는 §B 결정 2에서 확정한 대로 `/cases/new` 페이지 컴포넌트(`NewCasePage`) 자신의 서버측 세션 확인에서 전달되며, 이 함수 자신은 세션을 조회하지 않는다(순수 owner-scoped read 함수, 인증은 호출자 책임). 이는 §A에서 명시한 PRESERVE 목록의 유일한 예외이며(기존 파일 수정이 아닌 신규 파일 추가), 되돌릴 경우 REQ-013 전체가 무효화된다.
7. **반응형 및 모바일 드로어 접근성 구현 방식 — CSS 유틸리티 + 네이티브 React state (라이브러리 도입 없음, 조건부 재검토)**: 태블릿/모바일 규칙은 Tailwind v4 반응형 유틸리티(`md:`/`sm:` 등 프로젝트 기존 브레이크포인트 관례 확인 후 적용)와 CSS `transition`만으로 구현한다. 드로어 접근성 계약(ESC/스크림클릭/포커스이동복귀/스크롤잠금/tab순서/브레이크포인트 리셋 + 포커스 트랩/배경 비활성화, REQ-017 신규)은 `useState`/`useRef` + 표준 DOM 이벤트로 구현하며, 신규 포커스 트랩 전용 패키지를 도입하지 않는다(`package.json` 직접 조사로 미설치 확인). 단, run-phase에서 `pnpm install` 후 이미 설치된 `@base-ui/react`가 포커스 관리 내장 Dialog 계열 서브패스를 실제로 제공함을 확인하면, 그것을 우선 재사용해 직접 구현량을 줄인다(잔여 위험, research.md §10, §F4). **포커스 트랩(Tab/Shift+Tab 닫힌 루프) + 배경 `inert` + 닫을 때 포커스 복귀는 M8의 명시적 구현 책임이다** — 네이티브 구현이든 `@base-ui/react` Dialog 프리미티브 재사용이든, 이 세 가지는 run-phase 구현자의 재량으로 남겨지는 항목이 아니라 M8 완료 조건(acceptance.md AC-017i~AC-017l)에 명시적으로 포함된다. 프리미티브를 재사용하는 경우에도 자동화 테스트는 프리미티브의 존재만으로 통과 처리해서는 안 되며, 실제 동작(포커스 순환, 배경 상호작용 차단)을 직접 관찰해 검증해야 한다.
...
- M2 완료 시 `git diff app/cases/layout.tsx`로 `getCurrentSession()` 등 세션 동적 API 호출이 추가되지 않았음을 확인(§E 리스크). `/cases/new`의 정적/동적 렌더링 여부는 M2의 완료 조건이 **아니다** — 그 판정은 M6(REQ-013 최근 리서치)이 `NewCasePage` 자신에 서버측 세션 확인을 도입한 이후에만 의미가 있다(§B 결정 2, M6 참고)
- App Topbar에 spec.md REQ-006이 정의하는 정확한 라우트→브레드크럼/타이틀 매핑(`/cases/new`→"사건 입력", `/cases/[caseId]`→"리서치 리포트", `#expert-feedback`은 인페이지 앵커일 뿐 타이틀 불변)을 추가(비고정 유지)
...
- `app/cases/new/page.tsx`(`NewCasePage`)를 async Server Component로 전환해 `getCurrentSession()`으로 `ownerUserId`를 확인·확보하고, 위 조회 함수에 전달(§B 결정 2/6) — 인증된 세션이 없으면 기존 인증-리다이렉트 흐름을 따른다
- "최근 리서치" 패널(`case-recent-research`) 연결, owner-scope 단위 테스트 작성, 조회 실패 시 그 패널만 안전한 빈/오류 상태로 대체되고 폼 나머지는 정상 렌더링됨을 테스트로 검증
- M6 완료 시 `pnpm build` 실행 로그로 `/cases/new`가 Dynamic(요청 시점) 세그먼트로 표시됨을 확인하고, 빌드 자체가 실제 DB 연결을 요구하지 않고 성공함을 확인(spec.md REQ-013 빌드-안전성 요구사항, acceptance.md AC-005a)
...
- 390px 모바일 드로어 구현: 열기(`mobile-nav-toggle`)/닫기 버튼/스크림(`mobile-nav-scrim`)/ESC/포커스 이동·복귀/스크롤 잠금/닫힘 상태 tab-순서 제외/1024px 이상 리사이즈 자동 초기화 + 포커스 트랩(Tab/Shift+Tab 닫힌 루프)/배경 `inert` 비활성화 — §B 결정 7(구현 책임 명시), acceptance.md AC-017i~AC-017l
...
- `app/cases/layout.tsx`(서버 컴포넌트) 자신이 `getCurrentSession()` 등 동적 API를 직접 호출하는 것 금지(§B 결정 2 — 개별 페이지, 특히 `NewCasePage`가 REQ-013을 위해 자신의 서버측 세션 확인을 스스로 수행하는 것은 이 금지의 예외가 아니라 애초에 이 금지의 범위 밖이다: 금지 대상은 오직 레이아웃 자신이다)
...
| M2(사용자 블록 클라이언트 분리)를 잘못 구현해 `app/cases/layout.tsx`가 세션 동적 API를 직접/간접 호출(예: 클라이언트 컴포넌트를 서버 컴포넌트 안에서 잘못 조합) — 레이아웃이 감싸는 라우트의 렌더링 모드가 레이아웃 자신의 선택이 되어버리는 것을 방지 | M2 완료 시 `git diff app/cases/layout.tsx`로 `getCurrentSession()` 등 동적 API 호출이 추가되지 않았음을 확인 |
...
| M6(`NewCasePage`의 신규 서버측 세션 확인)이 의도치 않게 빌드 시점에 DB 연결을 시도하게 만들거나, 세션 확인 로직이 페이지 최상위가 아닌 여러 곳에 흩어져 동적 렌더링 경계가 불명확해짐 | M6 완료 시 `pnpm build` 실행 로그에서 `/cases/new`가 Dynamic으로 표시되고 빌드 자체는 실제 DB 연결 없이 성공함을 확인(AC-005a), 세션 확인은 `NewCasePage` 최상위에서 단 1회만 수행되는지 코드 리뷰로 확인 |
...
| M6(최근 리서치 조회 실패)이 패널 단위로 격리되지 않아 사건 입력 폼 전체가 렌더링에 실패함 | 조회를 별도 `try`/`catch` 또는 격리된 데이터-페칭 경계로 감싸 그 실패가 좌측 폼 컬럼에 전파되지 않음을 단위 테스트로 검증(spec.md REQ-013 폴백 동작) |
...
| M8(드로어 접근성)이 포커스 트랩을 불완전하게 구현해 접근성 회귀 발생 | AC-017a~AC-017l(포커스 트랩/배경 `inert` 포함) 각각을 개별 자동화 테스트로 작성, run-phase에 `@base-ui/react` Dialog 서브패스 실존 여부를 우선 확인(§F4) — 프리미티브를 재사용하는 경우에도 실제 동작을 직접 관찰해 검증(§B 결정 7) |
...
## §F. 열린 판단 (구현 시점 결정 — NEEDS-CLARIFICATION 마커가 아닌 재량 범위 명시)
...
- **F4**: `pnpm install` 이후 `@base-ui/react`의 Dialog 계열 서브패스(예: `@base-ui/react/dialog`)가 실제로 존재하고 포커스 관리를 내장하고 있는지 run-phase 착수 시 확인한다(research.md §10 잔여 위험). 존재하면 M8 드로어 구현에서 그 프리미티브를 우선 재사용해 직접 구현량을 줄인다. 존재하지 않거나 이 SPEC의 접근성 계약(REQ-017)과 맞지 않으면 §B 결정 7의 네이티브 구현으로 진행한다 — 어느 경우든 신규 패키지 설치는 하지 않는다. **어느 경로를 택하든 AC-017i~AC-017l(포커스 트랩/배경 `inert`)의 자동화 테스트는 프리미티브의 존재 여부가 아니라 실제 관찰된 동작(Tab/Shift+Tab 순환, 배경 요소 포커스·상호작용 차단)을 검증해야 한다 — "프리미티브가 있으니 당연히 동작한다"는 가정은 허용되지 않는다.**

## §G. Cross-references

- spec.md §2 REQ-001~024 — 이 계획이 구현하는 요구사항
- design.md §1~5 — 이 계획의 각 마일스톤이 참조하는 정확한 시각 스펙
- research.md §1~12 — M1~M8이 보존해야 할 기준선 및 REQ-005/REQ-011/REQ-013/REQ-015의 스코프·결정 근거(§5c — `/cases/new` 동적 렌더링 전환 메커니즘)
