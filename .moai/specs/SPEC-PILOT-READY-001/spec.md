...
- 2026-09-14 (v0.15.0): 범위 정합화 — Netlify Free의 동기 함수 60초 상한(§I 실측,
  §R 진단)에 부딪혀 §R(commit `ff706c2`, 2026-09-12)에서 `POST /api/cases`를
  `202`+jobId 즉시 응답 + Background Function(`netlify/functions/
  process-case-background.ts`) 비동기 처리로 전환했고, Gemini 무료 티어
  Research 모델 20 RPD 병목(§Y 실측)에 부딪혀 §하이브리드 라우팅 커밋
  (`187afc2`, 2026-09-13)에서 Researcher를 일반 사건은 Lite로, 기왕증/상해·질병
  혼재/추가확인 신호가 있는 사건이나 Lite 결과가 근거 대비 불충분한 사건만
  Premium으로 승격하도록 변경했다 — 둘 다 이 SPEC의 구현 중 실측으로 발견된
  운영 제약에 대한 정당한 범위 변경이며, REQ-PILOT-READY-007(4)/(008)/(010)에
  반영한다. `lib/pipeline/**` PRESERVE 제약(§E)도 이 범위 변경을 반영해
  정정한다(6단계 알고리즘 경계와 공개 타입 계약은 무변경, 라우팅 계층만
  추가). progress만으로 이 범위 변경이 승인된 것으로 처리하지 않는다 — 이
  HISTORY 항목 자체가 spec.md 본문 개정이다. 최신 HEAD(`3f0859b`) 기준
  로컬 품질 게이트 재검증(vitest 67/67·463/463, tsc/eslint/prettier/build
  PASS) + 실제 Deploy Preview·원격 Turso 대상 재검증(정상/복합 하이브리드
  스모크, 서로 다른 사용자 3명 동시 부하, 동일 사용자 경합·중복 실행
  claim·리스 재획득·지연 완료 fencing)을 완료했다 — 상세 근거는
  `.moai/specs/SPEC-PILOT-READY-001/progress.md` §Z/§AA, readiness-decision
  문서 이력 참고.
...
| ID | 유형 | 요구사항 | 근거 |
|----|------|----------|------|
| REQ-PILOT-READY-010 | When(이벤트 감지) | When 파일럿 런칭 준비 절차가 수행되면, 현재 main HEAD(이 SPEC의 run-phase 시점 기준) 또는 그 이후 커밋에 대해 실 Gemini 스모크 테스트(`.moai/reports/gemini-runtime-smoke-20260828.md`와 동일한 방법론 — 코드 임시 수정 없음, 관측기 기반 logical call count 확인)를 재실행하고 새 리포트로 남겨야 한다. 스모크는 최소한 다음 5가지를 개별적으로 확인·기록해야 한다: (a) Researcher/Skeptic/Verifier 3단계 각각의 실제 Gemini 호출 성공 여부, (b) `POST /api/cases`가 `202 Accepted`와 jobId를 반환하고 상태 조회가 완료된 사건의 `caseId`를 제공하는지, (c) 응답에 포함된 결과가 실제로 DB(`reports` 테이블)에 영속화됐는지, (d) 그 DB 행을 별도 조회로 재확인할 수 있는지(응답 값과 DB 값의 일치), (e) 관측된 Gemini 실제 호출 횟수가 파이프라인이 요구하는 기대 호출 횟수와 일치하는지 — 이 5가지 중 어느 하나라도 확인 없이 "스모크 통과"로 리포트에 기록해서는 안 된다. **(v0.15.0 정정)** 하이브리드 라우팅(`lib/pipeline/hybrid-research-router.ts`) 도입 이후 기대 호출 횟수는 더 이상 고정 3회가 아니다 — 일반 사건(Lite 시작, 승격 없음)은 3회(Lite Researcher+Skeptic+Verifier), 복합 신호 사건(기왕증/상해·질병 혼재/추가확인, Premium 직행)은 3회(Premium Researcher+Lite Skeptic+Verifier), Lite→Premium 승격이 발생한 사건은 최대 6회(Lite 라운드 3회+Premium 라운드 3회)다 — 리포트는 실행한 사건이 어느 경로였는지와 그 경로의 기대 횟수를 함께 명시해야 한다. | 사용자 지시(마지막 실 Gemini 증거가 13일·4개 SPEC만큼 stale, 스모크 검증 항목 구체화), `.moai/reports/gemini-runtime-smoke-20260828.md`(2026-08-28 시점, `feat/SPEC-GEMINI-RUNTIME-001@fb23553` 기준 — 이후 SPEC-EVIDENCE-001/SPEC-FEEDBACK-001/SPEC-PILOT-UX-001/SPEC-PILOT-VISUAL-001/SPEC-UI-MIGRATION-001/SPEC-E2E-AUTH-STATE-001가 병합됨) |
