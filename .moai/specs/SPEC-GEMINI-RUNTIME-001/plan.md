...
- 기존 `getLLMProvider(env)` 호출부(단위 테스트 stub 포함)를 `getLLMProviders(env)` 형태로 갱신. `provider-factory.test.ts` 전면 재작성 필요 — model-ID 동일/상이 두 경로 모두 커버.
- `rate-scheduler.test.ts`(신규): fake clock/sleep으로 동일 스케줄러에 대한 최소 간격 검증, budget 값 검증.
- REQ 커버리지: REQ-GEMINI-RUNTIME-001, REQ-GEMINI-RUNTIME-002, REQ-GEMINI-RUNTIME-003, REQ-GEMINI-RUNTIME-011, REQ-GEMINI-RUNTIME-012, REQ-GEMINI-RUNTIME-022.
