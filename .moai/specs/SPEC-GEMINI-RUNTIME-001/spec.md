...
- 2026-08-27: 외부 독립 리뷰 지적사항 반영(4차 개정, 1개 blocking 결함) — D-NEW1(`RateScheduler` 상태가 사건 경계를 넘지 못하는 결함): `getLLMProviders(env)`가 호출마다 새 `GeminiProvider`/`RateScheduler` 쌍을 만들고 `runPipeline()`의 정상 앱 경로가 사건마다 이 함수를 다시 호출하는 구조라, 사건 A→B 전환 시 자체 부과 RPM 페이싱이 "사건 하나 내에서만" 유효한 범위로 축소되던 결함을 발견·수정. `provider-factory.ts`에 프로세스 생애주기 지연 초기화 싱글턴 `getDefaultLLMProviders()`를 신설(모듈 스코프 `defaultProviders` 캐시, 최초 호출 시에만 `getLLMProviders(process.env)` 실행)하고, `runPipeline()`의 정상 경로가 이를 재사용하도록 재설계 — 명시적 `options.providers` 테스트 주입 경로는 이 싱글턴과 완전히 독립적으로 유지(design.md §1 D-NEW1). REQ-GEMINI-RUNTIME-025 신설(Tier L REQ 상한 25개 정확히 도달). D1~D5의 동일-model-ID 스케줄러 공유 계약(REQ-GEMINI-RUNTIME-022)은 무변경 — 이 싱글턴은 `getLLMProviders()`를 감싸는 얇은 래퍼일 뿐이다.
- 2026-08-27: plan-auditor Cycle 3 iteration 1 FAIL(score 0.92) 수정(소규모 델타) — D1(필수, blocking): REQ-GEMINI-RUNTIME-025가 명시한 두 절반(정상 경로의 싱글턴 재사용 / 테스트 주입 경로의 격리) 중 후자에 대응하는 AC가 §E 매트릭스에 전혀 없었던 결함을 acceptance.md에 AC-GEMINI-RUNTIME-016b 신설로 해소(테스트 주입 경로가 싱글턴을 전혀 호출하지 않는지, 그리고 서로 다른 명시적 `RoleProviders` 간 `RateScheduler.lastStartedAt` 누수가 없는지 검증). D2(선택, cosmetic): REQ-GEMINI-RUNTIME-025의 GEARS 유형 라벨을 `Ubiquitous`에서 이 문서의 기존 관례(REQ-008/014/016/024)와 일치하는 `Ubiquitous + Unwanted`로 재조정.
- 2026-08-27: 외부 독립 리뷰 지적사항 반영(5차 개정, 1개 소규모 델타 — 그라운딩 계약 회귀 방지) — 현행 `lib/pipeline/researcher.ts`의 per-query `buildFindingSchema()`는 `supportingEvidenceIds: z.array(z.string()).min(1)`로 Researcher finding이 최소 1개의 실제 evidence ID를 인용하도록 이미 보증하고 있는데, 이번 SPEC의 배치 전환 설계(design.md §2 옵션 2 — "구조는 스키마로, 업무 규칙은 파싱 후 코드로")가 스키마에서 `.min(1)`을 제거하면서 그 보증을 사후-파싱 업무 규칙 검증 순서에 명시적으로 재배치하지 않았던 결함을 식별·수정. REQ-GEMINI-RUNTIME-007 본문에 파싱 후 항목별 업무 규칙 검증의 정확한 5단계 순서(candidate 소속 확인 → 중복 응답 처리(첫 응답 채택) → **Researcher 항목에 한해** `supportingEvidenceIds.length >= 1` 검증(위반 시 그 항목만 폐기) → evidence ID 부분집합 검증 → safety-validator)를 명시하고, Skeptic 항목은 이 3단계를 건너뛰어 기존과 동일하게 빈 evidence 배열을 허용하는 비대칭임을 명확히 했다(design.md §2). Zod 스키마 자체는 여전히 구조만 검증하며(`.refine()`/`.min()` 없음) — 이는 D2가 확립한 부분 배치 실패 설계(item-레벨 refine도 배열 전체를 실패시키는 Zod atomic 특성 때문에 채택할 수 없음)와 정합하며, 새 검사는 파싱 성공 이후 애플리케이션 코드의 사후-검증 단계로만 추가된다. acceptance.md에 AC-GEMINI-RUNTIME-009a를 신설해 이 계약(빈 evidence finding만 개별 폐기, 같은 배치의 다른 정상 항목은 보존)을 직접 검증한다. Requirement A(모델 분리)/Requirement C(rate scheduler)/그 외 무관 섹션은 무변경.
...
- **Gemini 호출 배치**: Researcher·Skeptic이 쿼리/finding 개수만큼(N회) 호출하던 것을, Verifier가 이미 쓰는 "사건당 1회 구조화 호출" 패턴을 따라 사건당 1회로 줄인다. 목표: 정상 사건 1건당 핵심 **논리적** Gemini 호출 약 3회(Researcher×1 + Skeptic×1 + Verifier×1), 쿼리/finding 개수와 무관 — 429/503 재시도로 인한 **실제 HTTP 시도 횟수**는 이와 별개이며 이를 초과할 수 있다(§2 용어 구분 참고). 이 배치 전환은 기존 evidence 그라운딩 계약을 약화하지 않는다 — **Researcher finding은 최소 1개의 실제 query-evidence ID를 인용해야 한다**는 요구는 배치 전환 이후에도 그대로 유지되며(REQ-GEMINI-RUNTIME-007), 위반한 항목만 개별 폐기되고 배치 내 다른 정상 항목은 영향받지 않는다.
- **Free-tier rate scheduler(1차 방어선)**: **실제로 확정된 model ID** 단위로 독립적인 자체 요청 예산(RPM budget, Google의 실제 쿼터를 대신 주장하지 않는 self-imposed 값)으로 요청 시작 간격을 페이싱하며, 두 역할이 같은 model ID를 가리키면 하나의 스케줄러를 공유하고 더 보수적인 예산을 적용한다. 스케줄러는 최초 시도뿐 아니라 429/503 재시도로 인한 모든 후속 실제 호출 시도에도 동일하게 적용된다. 정상 앱 경로는 프로세스 생애주기 동안 재사용되는 provider 싱글턴을 통해 이 페이싱 상태를 사건과 사건 사이에도 계속 이어간다(사건마다 스케줄러가 새로 초기화되지 않음).
...
| REQ-GEMINI-RUNTIME-007 | Ubiquitous | Researcher/Skeptic 배치 구조화 응답 검증은, 배치 내 개별 항목(query 또는 finding) 하나가 구조적으로 잘못되었거나 위조 evidence ID를 인용하더라도, 같은 배치 내 다른 정상 항목의 결과까지 함께 폐기해서는 안 된다 — 문제된 항목만 개별적으로 "해당 query/finding에 대한 결과 없음"으로 처리해야 한다. 구조 검증은 Zod 스키마로만 수행하며(`.refine()`/`.min()` 없음), 파싱 성공(`ok: true`) 이후 애플리케이션 코드가 각 항목에 대해 다음 순서로 업무 규칙을 개별 적용한다: (1) candidate 소속(queryId/findingId) 확인 → (2) 중복 응답 처리(첫 응답만 채택) → (3) **Researcher 항목에 한해** `supportingEvidenceIds.length >= 1`(그 finding이 최소 1개의 실제 query-evidence ID를 인용해야 한다는 그라운딩 계약 — 위반 시 그 항목만 폐기; Skeptic 항목은 이 단계를 건너뛰고 기존과 동일하게 빈 evidence 배열을 허용한다) → (4) evidence ID 부분집합 검증 → (5) safety-validator(design.md §2 — 구조는 스키마로, 업무 규칙은 파싱 후 코드로). 단, 배치 응답 전체가 최상위에서 파싱 불가능한 진짜 구조적 실패는 그 호출 전체를 기존과 동일하게 fail-closed 처리해도 된다. | 사용자 지시 §B.1/§B.2, design tension 해소 지시, 외부 독립 리뷰 5차 지적(그라운딩 계약 회귀 방지) |
...
| REQ-GEMINI-RUNTIME-025 | Ubiquitous + Unwanted | `options.providers`가 명시적으로 주어지지 않은 `runPipeline()`의 정상 앱 실행 경로는, 프로세스 생애주기 동안 지연 초기화(lazy-init)되는 단일 `RoleProviders` 싱글턴(`getDefaultLLMProviders()`)을 재사용해야 하며, 사건마다 새로운 `GeminiProvider`/`RateScheduler` 쌍을 생성해서는 안 된다. 그 결과 `RateScheduler`의 `lastStartedAt` 페이싱 상태는 사건 A가 끝나고 사건 B가 시작되는 경계를 넘어 프로세스 생애주기 동안 보존되어야 한다. `options.providers`를 명시적으로 주입하는 테스트 경로는 이 싱글턴과 독립적으로 자신만의 `RoleProviders`를 구성해야 하며, 이 싱글턴 상태와 결합되어서는 안 된다. | 외부 독립 리뷰 4차 지적(D-NEW1), design.md §1 |

REQ 개수: 25개 (Tier L 상한 25개 — 정확히 도달, 초과 없음).
...
- **프로세스 생애주기 싱글턴이 개발 환경 hot-reload에서 리셋될 수 있음**: `getDefaultLLMProviders()`(REQ-GEMINI-RUNTIME-025)의 모듈 스코프 캐시는 Next.js dev 서버의 모듈 재평가(hot reload)로 초기화될 수 있다 — 개발 환경에서만의 사소한 위험이며(리셋돼도 안전 방향으로만 작동: 최악의 경우 "사건 경계 페이싱 기억이 한 번 끊긴다"이지 오류나 데이터 손상이 아니다), 프로덕션(`pnpm build && pnpm start`)에서는 문제되지 않는다. 여러 서버리스 인스턴스에 걸친 공유는 이 싱글턴의 범위 밖이며(§4 동시성 제한과 동일한 프로세스 로컬 한계), 별도로 주장하지 않는다.
