...
- 2026-08-27: post-run 코드 리뷰 1차 — merge-blocking 결함 4건 + 조사 1건 수정 (Fix-A~D) (Nexsol) — 커밋 `d03a00d`~`f773e39`(완료 기록 `4345c74`). M1~M6 구현 완료 이후, 새 SPEC을 만들지 않고 동일 SPEC에서 처리한 결함 수정 3차례 중 1차. Fix-A: `lib/pipeline/safety-validator.ts` 신설 — 보험금 지급확정/반드시 지급/숫자%·확률/근거없는 액수 확정 표현을 정규식으로 차단하는 공용 validator를 `researcher.ts`에 적용. Fix-B: `verifier.ts`가 evidence-ID 존재 여부만 검사하던 결함 수정 — provider 기반 의미 검증(semantic verification) 1단계 도입, safety-validator를 최종 출력에도 defense-in-depth로 재적용. Fix-C: `page.tsx`에서 `VerifiedClaim.status`가 표시되지 않던 결함 수정 — VERIFIED/INSUFFICIENT 배지 + 판단 불충분 사유 섹션 추가. Fix-D: `db/seed/evidence.json`의 evidenceType 라벨 정합화(DISPUTE_CASE → OTHER). 5종 게이트(test/lint/format/build/e2e) 전부 exit 0 확인.
- 2026-08-27: post-run 코드 리뷰 2차 — merge-blocking 결함 3건 수정 (item 1~3) (Nexsol) — 커밋 `0b5b3ae`. item 1: `verifier.ts`의 semantic verification을 Skeptic 반론(counterArgument)에도 적용 — 사건당 구조화 호출 1회 원칙 유지, 반환 계약을 `supported:boolean`에서 `supportedEvidenceIds`/`counterEvidenceIds`(실제로 의미 검증을 통과한 evidence ID의 부분집합) 반환으로 변경. item 2: Skeptic `challenge()` 생성 결과 + Verifier 최종 safety scan에 safety-validator를 status와 무관하게 적용, 금지 표현이 있는 counterArgument는 최종 배열에서 제거. item 3: 퍼센트 safety 규칙을 "지급/성공/승인/받을 확률·가능성 + 수치"로 좁혀 장해지급률·ROM 제한율·기왕증 기여도 같은 정상 수치는 허용, 지급액 확정 표현 규칙은 억/만원 단위 + 다양한 확정 어미를 포괄하도록 확장. 5종 게이트 전부 exit 0 확인(208/208 테스트).
- 2026-08-27: post-run 코드 리뷰 최종(3차) — 정합성 결함 2건 수정 (Nexsol) — 커밋 `f5a97b7`. item 1: semantic verification 전체 실패(`ok:false`) 시 claim candidate의 `supportingEvidenceIds`도 함께 비워 counterArgument의 fail-closed 처리와 대칭시킴. item 2: Verifier 최종 safety scan에서 claim summary 검사의 `status === "VERIFIED"` 게이팅을 제거해 실제로 status와 무관하게 수행되도록 정정. 부수 발견(회귀 없음 확인 후 수정): `lib/ai/providers/deterministic.ts`의 evidence-ID 추출 정규식이 summary/반론 텍스트 안 대괄호까지 오인식하던 잠재 결함을 함께 수정. 5종 게이트 전부 exit 0 확인(208/208 테스트).
