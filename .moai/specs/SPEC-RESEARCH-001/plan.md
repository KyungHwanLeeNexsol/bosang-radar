...
- `lib/pipeline/mock-llm.ts` 폐지, 참조하던 3개 파일(researcher/skeptic/verifier)의 import를 `deterministic.ts`로 교체(M5에서 각 단계 재작성과 함께 처리). 단, 2차 revision(항목 2)에서 provider는 세 함수 모두 필수 인자로 바뀌므로, 3개 파일은 더 이상 `deterministic.ts`를 자체 기본값(default parameter)으로 갖지 않는다 — `deterministic.ts`는 `provider-factory.ts`(정상 앱 경로)와 각 단위 테스트/E2E(명시적 주입)에서만 참조된다.
...
- `lib/pipeline/skeptic.ts`: 반론 생성 계약(기왕증/퇴행성/인과관계 부족 등 프롬프트 지시), `generateStructured()` 적용. `evidenceMap` 인자 추가(2차 revision, 항목 5) — Skeptic이 finding 텍스트뿐 아니라 실제 evidence를 프롬프트에 받고, `Challenge.supportingEvidenceIds`/`counterEvidenceIds`를 `buildChallengeSchema(validEvidenceIds)`(design.md §7)로 1차 방어선을 적용한다. provider는 필수 인자.
- `lib/pipeline/verifier.ts`: `evidence` 인자 추가, 2차 방어선(existing evidence ID 재검증 — Researcher claim **및** Skeptic evidence 양쪽 모두, design.md §7) + `VerifiedClaim.status`(`VERIFIED`|`INSUFFICIENT`) 산출, `missingMaterials` 생성 로직. provider는 필수 인자.
...
- `verifier.test.ts`의 기존 호출은 **4인자 필수 형태**(`verify(findings, challenges, evidenceMap, provider)`)로 재작성해야 한다(design.md §3 — `verify()`에 `evidence`가 3번째로 추가되고 `provider`가 4번째 **필수** 인자가 됨, 기본값 없음) — stub provider 객체의 형태만 바꾸는 것이 아니라, 컴파일을 깨는 시그니처 변경이므로 모든 호출부를 함께 고친다.
