...
- 2026-08-29: 외부 독립 리뷰(run-phase 중 발견된 측정방법론 blocker 2건) 반영(v0.5.0 → v0.6.0, run-phase 중 manager-develop이 blocker로 반환 → orchestrator가 manager-spec으로 재위임한 scope-doc 정정 — SPEC 상태는 `in-progress` 유지, 전면 재계획 아님). **측정방법론 이슈 1**(design.md §2.2/§3.4A, spec.md REQ-EVIDENCE-016/REQ-EVIDENCE-009): design.md §2.2(`computeScore()`)가 M2 탐색적 전략 A/B 비교와 §3.4A의 M4d frozen 최종 비교 양쪽에 동일하게 적용된다고 읽혀, §3.4가 이미 명시한 "baselineRetriever는 issueType 가중치 없음"이라는 요건과 내적으로 모순됐던 결함 — §2.2에 M2 exploratory scoring과 M4d baseline function이 서로 다른 함수임을 명시하는 caveat를 추가하고, §3.4A에 M4d 전용 `trueBaselineEligible()`/`trueBaselineScore()`(§2.1 `relevantA()`와 동일한 eligibility, issueTypeWeight를 구조적으로 참조할 수 없는 별도 score 함수)를 신설해 `computeScore()`/`relevantB()`와 이름으로 구분했다. REQ-EVIDENCE-016에 "baseline"이 가리키는 대상(true pre-SPEC 알고리즘, M2의 exploratory 전략 A 변형과 별개)을 명시하고, REQ-EVIDENCE-009에도 같은 naming 구분을 확인차 추가했다(실질 eligibility 식은 무변경). **측정방법론 이슈 2**(design.md 신규 §3.1a, spec.md REQ-EVIDENCE-016): M2의 `evidence-retriever.benchmark.test.ts`가 `db/seed/evidence.json`을 라이브 import해 구현되어(실측: M2 완료 시점 10건, 이후 M4 파일럿이 19건으로 확장) 이미 기록된 "M2 exploratory 10건 baseline" 수치가 재현 불가능해진 결함 — design.md §3.1a에 M2 벤치마크가 M1 시점 10건을 얼린 물리적으로 별도인 불변 fixture 스냅샷(라이브 import 금지, M4d의 frozen 최종 corpus와는 다른 종류의 불변성)을 사용해야 한다는 요건을 신설(REQ-EVIDENCE-014 범위, 새 top-level REQ 미추가)하고, REQ-EVIDENCE-016에 위반 시 과거 측정값을 "재측정 필요"로 재라벨링해야 한다는 조항을 추가했다. 두 이슈 모두 기존 REQ-EVIDENCE-009/014/016 wording 수정으로 통합했다 — REQ/AC 개수는 v0.5.0과 동일하게 25/25 유지(Tier L 상한 재확인), 새 REQ/AC ID 미추가. 이번 개정은 문서(spec.md/design.md)만 대상이며 코드/progress.md는 이 개정 세션에서 수정하지 않는다 — 재측정 자체는 후속 run-phase 작업이다.
- 2026-08-31: run-phase 중 발견된 baseline/fixture 모순 2건 반영(v0.6.0 → v0.7.0, run-phase 중 manager-develop이 blocker로 반환 → orchestrator가 manager-spec으로 재위임한 scope-doc 정정 — SPEC 상태는 `in-progress` 유지, 코드/테스트/progress.md는 이 개정 세션에서 수정하지 않는다). **모순 1(M2/M4d 역할 혼동이 실제 코드에 재발)**: v0.6.0이 design.md §2.2/§3.4A에서 "M2 exploratory scoring(`computeScore()`)"과 "M4d baseline(`trueBaselineEligible()`/`trueBaselineScore()`)"을 개념적으로 분리했음에도, `lib/pipeline/evidence-retriever.ts`의 production `retrieveEvidence()` 함수가 실제로는 `strategy === "A"`일 때 baseline score 함수(코드 구현명 `computeBaselineScore()`)와 tie-break 없는 정렬을 직접 배선해, REQ-EVIDENCE-012(production `retrieveEvidence()`의 결정론적 정렬)를 위반하고 design.md가 명시한 M2/M4d 역할 분리를 코드 수준에서 다시 무너뜨렸다 — design.md §2.1에 production `retrieveEvidence()`의 `strategy` 파라미터(값 "A"/"B" 둘 다)가 항상 §2.2 `computeScore()` + §2.3 결정론적 tie-break 정렬을 쓰며 오직 eligibility(`relevantA()`/`relevantB()`)만 다르다는 재발방지 확인 문단을 추가하고, §3.4A에 `trueBaselineEligible()`/`trueBaselineScore()`(및 tie-break 없는 정렬)는 `evidence-retriever.benchmark.test.ts`의 M4d 비교 절에서만 쓰이는 benchmark 전용 헬퍼(예: `trueBaselineRetrieveEvidence()`)로 격리해야 하며 production `retrieveEvidence()`의 `strategy` 분기에 배선해서는 안 된다는 명시적 금지 문단을 추가했다. REQ-EVIDENCE-012 wording을 이 재발방지 확인에 맞춰 정정했다(production `retrieveEvidence()`가 "A"/"B" 양쪽 모두에서 결정론적이어야 하며, tie-break 없는 정렬은 M4d benchmark 전용 헬퍼 안에서만 허용됨을 명시). **모순 2(empty ground truth 지표 pseudocode가 실제 구현과 불일치)**: design.md §3.3a의 `recallAt5()` pseudocode가 `knownRelevantIds.length === 0 ? 1 : ...`로 정의돼 있어 ground truth가 빈 benchmark case를 "완벽한 재현(Recall=1)"으로 잘못 채점하는 것처럼 읽혔으나, 실제 `evidence-retriever.benchmark.test.ts` 구현(progress.md §M Fix5)은 이미 empty ground truth에 대해 세 지표 모두 `null`을 반환하고 non-null case만으로 평균을 계산하며, empty ground truth case 수를 M4e(§3.4B) coverage gap으로 별도 보고하고 있다 — design.md §3.3a의 `recallAt5()`/`hitAt5()`/`precisionAt5()` pseudocode를 이 실제 구현과 정합하도록 수정(반환 타입을 `number | null`로, empty ground truth → `null`)하고, `null` 반환 시 평균 계산에서 제외되며 M4e coverage gap으로 보고된다는 설명 문단을 추가했다. spec.md/acceptance.md 전체를 grep해 이 외의 "empty GT → Recall=1" 류 서술이 남아있지 않음을 확인했다(design.md §3.3a 1건이 유일한 잔여 사례였다). 두 모순 모두 기존 REQ-EVIDENCE-012 wording 정정과 design.md §2.1/§2.2/§3.4A/§3.3a 정정으로 통합했다 — REQ/AC 개수는 v0.6.0과 동일하게 25/25 유지(Tier L 상한 재확인), 새 REQ/AC ID 미추가.
...
| ID | 유형 | 요구사항 | 근거 |
|----|------|----------|------|
| REQ-EVIDENCE-009 | Ubiquitous | `retrieveEvidence()`(`lib/pipeline/evidence-retriever.ts`)의 관련성 판정 술어(relevant predicate/candidate eligibility)를 현행 구조로 사전에 고정하지 않는다. run-phase는 최소 두 후보 전략 — **(A) 현행 전략**(domain 일치 + keyword 매칭만으로 후보 진입을 판정)과 **(B) 최소 확장안**(domain 일치 + keyword 매칭 또는 issueType exact match 중 하나라도 만족하면 후보 진입을 허용) — 을 동일 corpus·동일 벤치마크로 비교해야 한다. 정확한 최종 식(design.md §2.1)은 §D 벤치마크 결과를 근거로 확정하며, design.md 시점에 하나로 못박지 않는다. issueType 불일치를 이유로 evidence를 candidate 집합에서 무조건 배제하는 hard filter는 사전에 도입하지 않는다 — §D 벤치마크가 그런 필터가 실제로 필요하다고 뒷받침하지 않는 한, issueType은 배제(exclusion) 신호가 아니라 진입(inclusion-OR) 또는 정렬 신호로만 쓰인다(외부 독립 리뷰 이슈 1 — issueType이 ranking에만 쓰이고 eligibility에는 영향을 주지 못해, exact-issueType evidence라도 keyword가 없으면 애초에 후보 집합에 진입하지 못하는 결함 시정). **naming 참고(측정방법론 이슈 1 재확인)**: 여기서 "(A) 현행 전략"의 candidate eligibility 식은 design.md §3.4A가 정의하는 REQ-EVIDENCE-016의 M4d baseline eligibility(`trueBaselineEligible()`)와 동일한 식이지만, 그 두 맥락이 함께 쓰는 **score 함수**는 서로 다르다(M2 탐색적 비교는 design.md §2.2 `computeScore()`를 전략 A/B 공통으로 쓰고, M4d baseline은 별도의 `trueBaselineScore()`를 쓴다) — 이 REQ 자체는 eligibility만 다루므로 conflation이 없으나, "전략 A" = "baseline"이라는 이름을 score 함수까지 확대 해석해서는 안 된다(design.md §2.2/§3.4A). | 사용자 지시 §3, §1(리뷰) |
...
| REQ-EVIDENCE-012 | Ubiquitous | 동일 입력(evidence corpus 스냅샷 + query 집합)에 대해 production `retrieveEvidence()`(`lib/pipeline/evidence-retriever.ts`)의 결과는 `strategy` 파라미터 값("A"/"B" 둘 다 — REQ-EVIDENCE-009가 정의하는 candidate eligibility 선택 파라미터이며 score 함수를 바꾸지 않는다, design.md §2.1) 어느 쪽에서도 결정론적(같은 순서)이어야 한다 — score 동점 시 tie-break 규칙(예: `id` 오름차순)을 명시해야 한다. SPEC 착수 이전 main의 tie-break 없는(DB 행 순서 의존) 정렬은 §D의 M4d algorithm effect 비교 전용 benchmark-only 헬퍼(design.md §3.4A `trueBaselineEligible()`/`trueBaselineScore()`)에서만 재현하며, production `retrieveEvidence()`의 `strategy` 분기 코드 경로에 배선해서는 안 된다(외부 독립 리뷰 잔여 정합성 — 실제 코드에서 발생한 재발 사례, design.md §2.1/§3.4A). | 사용자 지시 §10 |
...
| REQ-EVIDENCE-016 | Ubiquitous | 최종 acceptance threshold 비교(**algorithm effect**)는 **하나의 freeze된 최종 corpus 스냅샷 + 하나의 freeze된 `BenchmarkCase`/`knownRelevantEvidenceIds`** 위에서, `baselineRetriever`(현행 알고리즘)와 `newRetriever`(§C에서 확정된 candidate/ranking 알고리즘)를 **동일한 corpus·동일한 ground truth**로 실행해 비교해야 한다 — corpus는 두 실행 모두 같고 알고리즘만 바뀌므로, 이 비교가 측정하는 것은 "최종 corpus에서의 algorithm effect"이지 corpus 확장 효과가 아니다(외부 독립 리뷰 v0.3.0 이슈 1). **여기서 `baselineRetriever`가 의미하는 "현행 알고리즘"은 issueType이 candidate eligibility에도 score 함수에도 전혀 관여하지 않는, 이 SPEC 착수 이전의 원래 pre-SPEC 알고리즘(design.md §3.4A의 `trueBaselineEligible()`/`trueBaselineScore()`)을 가리킨다 — M2(§C)의 탐색적 전략 A 비교(전략 A는 eligibility만 다르고, score 함수(design.md §2.2 `computeScore()`, issueTypeWeight 포함)는 전략 B와 공유하는 exploratory 변형)와는 서로 다른 대상이며, 이 둘을 혼동해서는 안 된다(design.md §2.2/§3.4A, 외부 독립 리뷰 측정방법론 이슈 1).** corpus가 아직 확장 중인 중간 시점(M2, 10건 corpus)의 측정값은 **exploratory baseline**으로만 기록하며, 최종 acceptance threshold의 직접 비교값으로 사용하지 않는다(외부 독립 리뷰 이슈 2). **이 M2 exploratory 측정은 design.md §3.1a가 정의하는, 물리적으로 별도인 불변 fixture 스냅샷(M1 시점 10건 — `db/seed/evidence.json`에 대한 라이브 import 금지) 위에서 수행해야 한다. 계속 확장되는 라이브 파일을 그대로 import해 측정하면 corpus 크기가 달라질 때마다 과거 "M2 exploratory 10건 baseline" 기록이 재현 불가능해진다(외부 독립 리뷰 측정방법론 이슈 2) — 이 요건을 위반한 채 이미 기록된 과거 측정값이 있다면, run-phase는 그 값을 progress.md에 "이후 corpus 확장의 영향을 받음 — 불변 스냅샷 기준 재측정 필요"로 재라벨링해야 한다.** Recall@5 / Hit@5 / Precision@5 세 지표를 함께 측정·기록해야 하며(design.md §3.3a/§3.3b), Recall@5만 보고하고 Precision@5 하락을 누락해서는 안 된다. 이 비교의 **기본 PASS 조건**은 (a) new(전략 B) Recall@5 ≥ baseline(전략 A) Recall@5, new Hit@5 ≥ baseline Hit@5, new Precision@5 ≥ baseline Precision@5라는 3개 지표 조건과 (b) REQ-EVIDENCE-013가 지정한 target case(exact issueType, no keyword)에서 baseline이 miss하고 new가 hit함이 실제로 관측되어야 한다는 target case 조건, 두 가지를 **모두** 만족해야 한다(design.md §3.3b). 측정 결과 (a)의 3개 지표 중 하나라도 trade-off(예: new Precision@5 < baseline Precision@5)로 미충족되면, 그 결과에 맞춰 threshold를 낮추거나 계약을 조용히 수정해 PASS로 만들어서는 안 된다 — score 함수 파라미터 재조정 또는 trade-off를 감수하는 명시적 근거(악화된 metric과 폭, 제품적 수용 사유, 검토한 대체 전략/weight 조정 결과)를 사람이 판단해 acceptance.md 또는 progress.md에 design exception으로 기록해야 하며, 그렇게 완화된 계약은 plan-auditor 재검토 대상이다(외부 독립 리뷰 잔여 정합성 이슈 1). **(b)의 target case 조건은 이 trade-off 예외 경로의 대상이 아니다** — target case 복구는 전략 B(§C) 채택의 존재 이유 자체이므로(design.md §3.3b), baseline miss → new hit이 관측되지 않으면 3개 지표가 전부 통과해도 이 REQ는 무조건 미충족이며, design exception으로 우회할 수 없다(외부 독립 리뷰 잔여 정합성 이슈 D8). | 사용자 지시 §4, §2(리뷰) |
