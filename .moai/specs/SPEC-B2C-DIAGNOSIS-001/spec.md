...
### 3.3 추가 질문 · 상태 전이 — 01-B/M01-B, 01-C/M01-C, 01-D, 01-E (Event-driven / Where)
...
- **REQ-B2CDIAG-013** (Where): 매칭되는 담보가 없는 것으로 판정되는 경우, 시스템은 결과 없음(01-D) 상태를 표시하고 입력 내용 수정 경로를 제공한다.
- **REQ-B2CDIAG-014** (Where): 분석 처리 자체가 실패하는 경우, 시스템은 분석 오류(01-E) 상태를 표시하며 입력 내용을 보존한 채로 "다시 시도"와 "입력 내용으로 돌아가기" 두 가지 복구 경로를 제공한다.
...
- **REQ-B2CDIAG-016** (Where): 프로덕션 환경에서 중간 상태(동의 완료 이후·추가 질문·진단 중·결과)에 해당하는 URL로 직접 접근하는 경우, 시스템은 선행 상태가 클라이언트 메모리에 없으면 01 초기 입력 화면으로 되돌린다.
...
- **REQ-B2CDIAG-019** (Where): 사용자가 추가 질문 화면에서 이전 단계(동의 화면)로 돌아가는 경우, 시스템은 이미 완료된 필수 동의 상태를 같은 세션 내에서는 유지한다.

### 3.5 기술 원칙 (Ubiquitous)

- **REQ-B2CDIAG-020**: 시스템은 01 진단 입력을 zod 스키마로 검증하며, 이름·전화번호·주민등록번호 형식에 해당하는 입력을 구조적으로 거부한다.
