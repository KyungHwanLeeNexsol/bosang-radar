...
두 요소(M01 안내 배너, M01-A2 설명)에 `expectLineTexts`를 추가하자마자
**M01-A2 설명이 디자인보다 한 글자 일찍 끊기고 있던 것**이 드러났다 —
구현 `…처리됩` / `니다.` vs 디자인 `…처리됩니` / `다.`. 8차까지 이 요소는
줄 수만 같으면 통과했다.

그리고 **8차 §7의 기술 자체가 스크립트보다 느슨했다**: M01 안내 배너는
"줄 수만 검증"이라고 적혀 있었지만 실제 설정에는 `expectLines`가 아예 없어
줄 수조차 검사하지 않았다. 문서를 코드에서 다시 세어 맞췄다(아래 "문서·코드
정합" 참고).
...
| M01 안내 배너 | "줄 수만 검증" | 실제로는 `expectLines` 없음 → 9차에 지점까지 추가 |
