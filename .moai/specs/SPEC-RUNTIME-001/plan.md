...
파일 수가 Tier M 밴드(5-15)를 초과하고, 인증 계정 생성이라는 보안 경계를 신설하므로 **Tier L**로 분류한다. 다만 아키텍처 변경이 없는 활성화 SPEC이므로 REQ 21 / AC 22로 상한(25/25) 대비 여유를 두고 타이트하게 유지한다.
...
### §A.4 플랜 개정 v0.4.0 (구현 착수 승인 전, 최종 정합성 점검)

감사 4회차 PASS(0.97) 이후, 사용자가 구현 착수 승인 직전 최종 검토를 요청해 **검증 범위 조정 1건 + 신규 안전장치 설계 1건 + 문서 정합성 정리 3건**을 반영했다. `spec.md`의 목표(WHY/WHAT)와 §4 제외 범위 6개 항목은 **변경하지 않는다**.

| # | 대상 | 개정 전 | 개정 후 | 영향 마일스톤 |
|---|------|---------|---------|---------------|
| 1 | AC-RUNTIME-022 검증 범위 | Then이 "Playwright 러너"와 "앱 서버" **양쪽**에 전달된 env를 직접 관측한다고 주장 | 관측 범위를 `run-e2e.ts`가 **직접 spawn하는 Playwright 러너 구간**으로 좁히고, `playwright.config.ts`의 `webServer.env` 미재선언 정적 검증을 추가. 앱 서버까지의 전파는 AC-RUNTIME-015가 기능적으로 커버함을 명시 | M5 |
| 2 | `.env.local` 안전장치 | 미정의 — sentinel/테스트 `.env.local`이 개발자의 실제 파일을 덮어써도 될지 unspecified | `design.md` §3.6 신설: 백업(트리 밖) → sentinel 기입 → 모든 종료 경로(성공/실패/SIGINT/SIGTERM/exit)에서 복원. 격리된 워크트리에서는 백업 자체 불필요. `kill -9`는 잔여 위험으로 정직하게 명시 | M5 |
| 3 | 문서 정합성 | REQ/AC 개수 잔존 표기, `e2e/global-setup.ts` 잔존 표현, `progress.md` §E.1 시제 모호성 | 정정(본 항목, `acceptance.md` AC-RUNTIME-011, `progress.md` §E.1) | — |

개정 1은 REQ-RUNTIME-016의 "동일 시크릿 공유 보장" 요구를 완화하지 않는다 — AC-RUNTIME-022(구조적, 진입점→러너 구간)와 AC-RUNTIME-015(기능적, 전체 흐름)가 함께 여전히 완전히 커버한다. **어떤 AC의 판정 기준도 낮추지 않았다.**
...
- **구조적 env 공유 검증 (신규 v0.3.0, 범위 조정 v0.4.0, REQ-RUNTIME-016 / AC-RUNTIME-022)**: `scripts/run-e2e.ts`가 **직접 spawn하는 유일한 자식 프로세스인 Playwright 러너**의 생성 지점을 **주입 가능한 형태**로 분리하고(예: spawn 함수를 인자로 받는 실행 함수), Vitest 단위 테스트가 **기록용 대역**을 주입해 그 호출에 전달된 env 객체를 수집·단언한다. 단언 대상: Playwright 러너에 전달된 env가 `BETTER_AUTH_SECRET`·`TESTER_PASSWORD`·`TURSO_DATABASE_URL`·`BETTER_AUTH_URL`에 대해 진입점이 조립한 값과 일치하며, DB URL이 `file:` 스킴일 것. 정적 보완으로 `playwright.config.ts`의 `webServer.env`가 위 네 키를 재선언하지 않음을 확인한다.
  - **범위 조정 이유 (v0.4.0)**: `run-e2e.ts`가 직접 spawn하는 자식은 Playwright 러너 하나뿐이며, 앱 서버(Next.js)는 Playwright 자신의 `webServer` 훅이 **Playwright 내부에서** spawn하므로 `run-e2e.ts`의 주입 가능한 spawn 지점으로는 가로챌 수 없다. 앱 서버까지의 실제 전파와 그 값으로 인증 흐름이 동작하는지는 AC-RUNTIME-015의 실제 Playwright E2E 실행이 기능적으로 검증하며, 이 AC는 그 대체물이 아니다.
  - **개정 이유 — 과잉주장 제거 (v0.3.0)**: 초판·v0.2.0은 "로그인 시나리오가 통과한다는 사실 자체가 시크릿 일치를 **입증한다**"고 기술했다. 이는 논리적 과잉이다 — 로그인 성공은 인증 흐름이 동작한다는 증거이지 특정 환경변수 **값의 동일성**에 대한 직접 증거가 아니며, "불일치했다면 실패했을 것"이라는 역추론은 로그인을 성공시킬 다른 경로(세션 쿠키 재사용, 캐시된 세션, 재시도)가 모두 배제되었을 때만 성립하는데 이 SPEC은 그 배제를 확보하지 않았다. 근본 문제는 **관측 지점과 주장 지점의 불일치**이므로, 주장이 "전달된 env가 같다"라면 관측도 전달된 env여야 한다(`design.md` §3.5).
  - **[HARD] Playwright 불필요**: 이 검증은 브라우저·앱 기동 없이 `pnpm test`에서 통과해야 한다(REQ-RUNTIME-018 유지). 로그인 성공 여부를 이 검증의 증거로 사용하는 것은 금지한다.
  - **검증 범위는 넓어진다**: 기존에 간접 추론뿐이던 구조적 성질에 직접 관측이 추가되고, Playwright 시험은 자신이 실제로 증명하는 것(인증 흐름의 동작)만 주장하게 된다. 관측 대상을 실제로 만들어낼 수 있는 범위(진입점→러너 구간)로 좁힌 것은 검증의 약화가 아니라 **주장 가능한 범위와 관측 범위의 정합**이다.
...
- **`.env.local` 안전 교체·복원 (신규 v0.4.0)**: AC-RUNTIME-015·AC-RUNTIME-021이 요구하는 sentinel/테스트 `.env.local` 상태는 개발자가 이미 보유했을 수 있는 실제 `.env.local`을 덮어써 만든다. `design.md` §3.6이 정의한 백업(트리 밖)→sentinel 기입→모든 종료 경로(정상/실패/`SIGINT`/`SIGTERM`/`exit`)에서 복원 절차를 구현한다. 격리된 워크트리에서 실행 중이면 물리적으로 다른 파일이므로 이 절차 자체를 생략한다. **`kill -9`는 닫히지 않는 잔여 위험**으로 남기고 완화를 주장하지 않는다(`spec.md` §5).
- **시나리오 파일**:
...
- **셀렉터 정책**: 현행 마크업 의존을 최소화하기 위해 `data-testid`를 필요한 지점에만 부착한다. UI 구조·스타일 변경은 금지(§A.5 PRESERVE).
...
- **AC-RUNTIME-022가 자신이 직접 spawn하지 않는 프로세스(앱 서버)에 전달된 env를 관측한다고 주장하기** *(신설 v0.4.0)* — `run-e2e.ts`는 Playwright 러너 하나만 직접 spawn한다. 앱 서버는 Playwright 내부에서 spawn되므로 그 지점을 가로챌 수 없다. 관측 범위는 진입점→러너 구간에 한정하고, 앱 서버까지의 전파는 AC-RUNTIME-015가 기능적으로 검증하게 한다.
- **sentinel/테스트용 `.env.local`을 쓴 뒤 개발자의 원본을 복원하지 않고 검증을 끝내기** *(신설 v0.4.0)* — `design.md` §3.6의 백업·복원 절차 없이 sentinel 값을 직접 덮어쓰면 개발자의 실제 자격증명이 유실된다. 모든 종료 경로(정상/실패/시그널/`exit`)에서 복원을 시도해야 하며, 격리된 워크트리가 아닌 한 이 절차를 생략할 수 없다.
