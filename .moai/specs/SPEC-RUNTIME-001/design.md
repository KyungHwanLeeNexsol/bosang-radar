...
| 층 | 검증 대상 | 수단 | AC |
|----|-----------|------|-----|
| **구조적** | `run-e2e.ts`가 **자신이 직접 spawn하는 Playwright 러너 프로세스**에 진입점이 조립한 값과 동일한 env를 전달한다 (앱 서버 프로세스는 Playwright의 `webServer` 훅이 내부적으로 spawn하므로 관측 대상 밖) | 단위 테스트 — Playwright 러너 생성 호출을 가로채 **전달된 env 객체를 직접 단언** + `playwright.config.ts`의 `webServer.env` 미재선언 정적 확인. Playwright·브라우저·앱 기동 **불필요** | AC-RUNTIME-022 |
| **기능적** | 실제 인증 흐름(폼 제출 → 세션 생성 → 보호 경로 접근)이 **동작한다** — 이는 필연적으로 Playwright 러너 → Next.js 서버 구간까지 값이 올바르게 전파되었을 때만 성립한다 | Playwright 로그인 시나리오 | AC-RUNTIME-011 / AC-RUNTIME-015 |

**구조적 검증의 형태 (범위 조정 v0.4.0)**: `scripts/run-e2e.ts`는 **자신이 직접 spawn하는 유일한 자식 프로세스인 Playwright 러너**의 생성 로직을 **주입 가능한 형태**로 분리한다(예: spawn 함수를 인자로 받는 실행 함수). 단위 테스트는 실제 spawn 대신 **기록용 대역**을 주입해, 그 호출에 전달된 env 객체를 수집한 뒤 다음을 단언한다:

- Playwright 러너 프로세스에 전달된 env가 `BETTER_AUTH_SECRET`·`TESTER_PASSWORD`·`TURSO_DATABASE_URL`·`BETTER_AUTH_URL`에 대해 진입점이 조립한 값과 **일치**한다(중간에 재조립·누락·침묵 변경 없음).
- (정적 보완) `playwright.config.ts`의 `webServer.env`가 위 네 키를 재선언하지 않는다 — 상속 경로 위에 값을 덮어쓰는 지점이 없어야, 위에서 확인한 값이 서버까지 이어지는 상속 경로에서 변조되지 않는다.

**이 검증이 앱 서버(Next.js) 프로세스에 전달된 env까지 관측하지 않는 이유**: `run-e2e.ts`는 Playwright 러너 하나만 직접 spawn한다. 앱 서버는 Playwright 자신의 `webServer` 훅이 **Playwright 내부에서** spawn하므로, `run-e2e.ts`의 주입 가능한 spawn 지점으로는 그 호출을 가로챌 수 없다 — 이전 버전(v0.3.0)의 AC-RUNTIME-022는 이 메커니즘의 한계를 넘어서는 관측(앱 서버에 전달된 env)을 주장했는데, 그 부분은 자신의 Given/When으로 만들어낼 수 없는 주장이었다. 앱 서버까지 값이 실제로 전파되고 그 값으로 인증 흐름이 동작하는지는 **AC-RUNTIME-015의 실제 Playwright E2E 실행**이 기능적으로 검증한다(위 표).

이 형태를 택한 이유는 **관측 지점을 주장 지점과 일치시키기 위해서**다. 주장이 "진입점이 러너에 전달한 env가 진입점 자신이 조립한 값과 같다"이므로, 관측도 그 전달 지점이어야 한다. 프로세스를 실제로 띄울 필요가 없어 `pnpm test`(Vitest)에서 브라우저 없이 실행되며(REQ-RUNTIME-018 유지), 실패 시 어떤 변수가 어떻게 갈라졌는지가 **즉시 드러난다** — 로그인 실패라는 간접 증상으로 나타나던 것과 대비된다.

> **프로세스 토폴로지 의존성**: 위 단언은 §3.4의 계보(`run-e2e.ts` → Playwright 러너 → Next.js 서버)를 전제하며, **관측 대상은 `run-e2e.ts` → Playwright 러너 구간에 한정**된다. 구현 시 실제 토폴로지가 다르게 확정되면(예: 서버를 진입점이 직접 spawn하도록 재설계) 단언 대상 호출 집합도 그에 맞춰 조정하되, **"진입점이 직접 spawn하는 자식에 전달한 env를 직접 관측한다"는 성질은 유지**하고 **자신이 직접 spawn하지 않는 프로세스에 대한 관측을 주장하지 않는다**(§6 구현 확정 항목).

**이 분리는 검증 범위를 넓힌다**: 기존에는 간접 추론만 존재하던 구조적 성질에 **직접 관측이 추가**되고, Playwright 시험은 자신이 실제로 증명하는 것(인증 흐름의 동작)만 주장하게 된다. 어느 쪽도 약화되지 않는다.

### 3.6 `.env.local` 안전 교체·복원 — 검증이 개발자의 실제 파일을 파괴하지 않는다 (신규 v0.4.0)

AC-RUNTIME-015(§3.4 sentinel 절)와 AC-RUNTIME-021(§3.2.2)의 Given은 모두 디스크의 `.env.local`이 특정 sentinel/테스트 내용을 담고 있을 것을 요구한다. 그러나 개발자의 머신에는 **이미 실제 개발용 자격증명이 담긴 `.env.local`이 존재할 수 있다** — 이 파일을 검증 목적의 sentinel/테스트 내용으로 덮어쓴 뒤 복원하지 않으면, 검증 절차 자체가 개발자의 작업 상태를 파괴하는 사고가 된다. 이 절은 그 파괴를 막는 메커니즘을 정의한다.

#### 기본 메커니즘 (항상 적용, [HARD])

sentinel/테스트 `.env.local`을 쓰기 전에, 프로젝트 루트에 이미 `.env.local`이 존재하면 **OS 임시 디렉터리**(프로젝트 트리 밖 — gitignore 대상 경로가 아니라 트리 자체 밖) 아래 고유한 이름으로 백업한다. 그다음 sentinel/테스트 `.env.local`을 쓴다. 검증 실행 후 — **성공·테스트 실패·프로세스 중단(`SIGINT`/`SIGTERM`) 모두에서** — 백업으로부터 원본을 복원한다. 검증 실행 전에 `.env.local`이 존재하지 않았다면, 실행 후 sentinel/테스트 파일을 삭제해 "존재하지 않던 상태"로 복원한다(남겨두지 않는다).

- **정상 종료(성공/실패 모두) 경로**: `finally`류 정리 블록에서 복원.
- **`SIGINT`/`SIGTERM` 경로**: OS 시그널 핸들러를 등록해 복원.
- **프로세스 `exit` 경로**: 동기식 최종 복원을 `process.on("exit", ...)`에 등록해, 위 두 경로가 모두 놓친 경우의 마지막 방어선으로 둔다.

**[HARD]** 어떤 종료 경로에서도 개발자의 원본 `.env.local`이 유실·손상되어서는 안 된다 — **단, `kill -9`(`SIGKILL`)처럼 인-프로세스 시그널 핸들러 자체를 우회하는 강제 종료는 이 설계로 닫을 수 없는 잔여 위험**으로 명시한다(`spec.md` §5, `acceptance.md` §D). 이는 이 SPEC이 이미 다른 곳에서 쓰는 정직성 원칙과 같다 — 닫히지 않는 경로를 닫힌 것처럼 서술하지 않는다(`research.md` §6 D1 Gap과 같은 서술 방식).

#### 우선 메커니즘 — 격리된 워크트리 (가능하면, SHOULD)

검증 실행 프로세스가 **격리된 git 워크트리**(별도 작업 트리 체크아웃 — 예: CI, 또는 `moai cc -w`를 사용하는 개발자)에서 실행 중이라면, 그 워크트리는 자신만의 별도 프로젝트 루트를 가지므로 `.env.local` 쓰기 대상이 개발자의 주 체크아웃에 있는 실제 `.env.local`과 **물리적으로 다른 파일**이다 — 이 경우 위 백업·복원 절차 자체가 필요 없다. 러너가 이미 격리된 워크트리 안에 있다면 이 경로를 우선하고, 아니라면 위 기본 메커니즘으로 폴백한다.

§3.2.2가 이미 확정한 설계 결정 — `loadEnvConfig`의 `dir` 인자가 cwd가 아니라 **프로젝트 루트로 고정**된다는 것 — 과 이 절은 상충하지 않는다. 그 결정이 막는 것은 "같은 체크아웃 안에서 cwd만 바꿔 다른 곳을 로드하게 만드는" 시도다. 여기서 말하는 워크트리 우선 메커니즘은 cwd 트릭이 아니라 **프로젝트 루트 자체가 물리적으로 다른** 별도 체크아웃을 전제하므로, `dir` 고정 결정을 우회하지 않는다 — 오히려 각 워크트리에서 `dir`이 각자의(물리적으로 다른) 프로젝트 루트를 가리키는 정상 동작일 뿐이다.
...
- **구조적 env 공유 검증의 대상 호출 집합 (v0.3.0 신설, 범위 조정 v0.4.0)** — §3.5의 단위 검증은 `run-e2e.ts` → Playwright 러너 → Next.js 서버라는 §3.4 계보를 전제하며, **`run-e2e.ts`가 직접 spawn하는 Playwright 러너 구간만 관측 대상**이다(앱 서버는 Playwright가 내부적으로 spawn하므로 관측 지점 밖). 구현 시 실제 프로세스 토폴로지가 다르게 확정되면 단언 대상 호출 집합을 그에 맞게 조정하되, **"진입점이 직접 spawn하는 자식 프로세스에 전달된 env 객체를 직접 관측한다"는 성질은 유지**하고 직접 spawn하지 않는 프로세스에 대한 관측은 주장하지 않는다. 로그인 성공을 시크릿 동일성의 증거로 되돌리는 형태는 허용하지 않는다(AC-RUNTIME-022).
- **`.env.local` 안전 교체·복원의 구현 세부 (신규 v0.4.0)** — §3.6이 정의한 백업·복원 메커니즘의 정확한 구현 수단(임시 디렉터리 API, 시그널 핸들러 등록 지점)은 구현 시 실측으로 확정한다. 설계상 확정된 것은 **경계 조건**(모든 종료 경로에서 복원 시도, 격리된 워크트리에서는 백업 자체 생략)이며, `kill -9`가 닫히지 않는 잔여 위험이라는 사실은 완화하지 않는다.
