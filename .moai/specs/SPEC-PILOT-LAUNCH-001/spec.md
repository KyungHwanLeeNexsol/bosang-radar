...
- 2026-09-14: run-phase 완료 후 외부 구현 검토 반영 (Nexsol, 검토 HEAD
  ...
  PR/병합은 아직 이루어지지 않았다.
- 2026-09-14: 외부 재검토 반영 (Nexsol, 검토 HEAD `ad9a92e`) — 문서 정확성
  정정 1건. account-provisioning.md §2-2의 "실제 로그인 세션은 프로덕션
  자신의 `BETTER_AUTH_SECRET`으로 서명한다"는 설명을 제거 — 현재 설정의
  기본 `session_token`은 DB 세션을 가리키는 opaque token이며
  `session.cookieCache`는 활성화돼 있지 않다. 결론(프로비저닝용 secret은
  Production과 일치 불필요 — scrypt 비밀번호 검증, `autoSignIn: false`로
  발급 시 세션 미생성)은 유지했다. "임의의 값"을 "프로덕션 값과는 별개인
  충분히 강한 실행용 값(예: `openssl rand -base64 32`)"으로, "조건은 두
  가지뿐"이라는 단정을 "핵심 운영 확인 항목은 다음과 같다"로 완화.
