...
- **Defect found + fixed same-day (CI/CD static-asset gap)**: after the CI/CD deploy (item above), the user reported broken CSS on `/login`. Root cause: Next.js `output: "standalone"` does NOT copy `.next/static/` or `public/` into `.next/standalone/` — the deploy workflow rebuilt `.next/` on every run but never re-synced those two folders into the standalone output, so hashed CSS/JS chunk filenames from the new build 404'd against the stale (or entirely absent) standalone copy. Fixed by (a) immediately copying `.next/static` + `public` into `.next/standalone/` on the live server and restarting PM2, and (b) adding the copy step (`rm -rf` + `cp -r`, idempotent) to `.github/workflows/deploy.yml` permanently, plus a smoke-check addition that fetches `/login`, extracts a referenced CSS chunk path, and verifies it actually returns 2xx — so a future regression of this exact class fails the deploy instead of shipping silently. Unresolved: whether this same gap existed since the M1 manual deploy (M1's own verification only checked HTML status code, not asset loading) is unknown and not worth root-causing retroactively — the fix + regression-catching smoke check cover it going forward.
