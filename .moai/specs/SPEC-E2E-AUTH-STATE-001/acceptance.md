...
>
> **개정 v0.1.5 (독립 감사 — 요구사항/구현 대조 + case-flow 회귀 조사, HEAD `167c71b` 대상)**: 기존 progress.md의 PASS 표기를 증거로 삼지 않고 REQ/AC를 코드·실행 로그와 처음부터 재대조했다. **(1) AC-E2EAUTH-009 pathspec 정정**: sync-phase가 `CHANGELOG.md`를 수정하면서 원래 pathspec으로는 그 diff가 비어 있지 않게 나타남을 발견 — REQ-009의 "애플리케이션/프로덕션 코드" 의도에 맞춰 `':!CHANGELOG.md'`를 pathspec에 추가(애플리케이션 코드 무변경 판정 자체는 불변). **(2) [HARD] case-flow.spec.ts 회귀 확인**: `$SPEC_START_SHA`에서 격리된 워크트리로 동일 조건 3회 재현한 결과, 베이스라인에서는 `case-flow.spec.ts`가 3/3 무재시도 통과, 반대로 `case-input-mobile-layout.spec.ts`/`tenant-isolation.spec.ts`가 3/3 모두 1차 실패 → retry로 회복(원래 SPEC이 기록한 flaky 대상과 정확히 일치)했다 — 이번 SPEC 이후에는 그 패턴이 뒤바뀌어 대상 2개 파일은 각각 6/6 무재시도(전체 스위트 5회 + 그 파일이 포함된 `--spec` 필터 1회 — **[v0.1.6 정정]** 과거 "7/7"은 두 파일 모두를 "독립 실행 7회" 분모로 계수한 과잉 집계였다. 각 파일은 자신을 포함하지 않는 반대쪽 필터 실행에서는 애초에 실행되지 않으므로 6회가 맞다), `case-flow.spec.ts`는 5/5(필터 실행에는 애초에 포함되지 않으므로 분모가 5) 전부 1차 실패 → retry로 회복한다. 이는 우연의 일치가 아니라 `e2e/auth.setup.ts`가 스위트 시작 직후 2회 로그인을 새로 추가해 Better Auth `/sign-in` rate-limit(10초 창 내 최대 3회) 예산을 기존보다 이른 시점에 소비하기 때문으로 원인이 특정된다(상세: `progress.md` § case-flow 회귀 조사). 이 SPEC의 어떤 AC도 이 사실만으로 FAIL 처리되지는 않는다 — AC-E2EAUTH-004는 "무재시도"가 아니라 "통과(재시도 허용)"만 요구하며, 대상 3항목(무재시도 요건)에 case-flow는 애초에 포함되지 않는다. 다만 이전까지 이 문서와 CHANGELOG.md가 이 사실을 "기존 flaky 그대로 유지"로 서술한 것은 부정확했으며, 이번 개정에서 "이 SPEC의 변경으로 rate-limit 충돌이 이동함"으로 정정한다. **(3) 실행 횟수 재계산**: v0.1.3 재검증은 전체 스위트 5회 + `--spec` 필터 2회 = 독립 실행 7회이며(015a의 leftover 재실행은 그 5회 중 2번째 실행을 재사용한 것으로, 별도 실행이 아니다), setup(TESTER_A/B) invocation 합계는 7×2=14회다 — 과거 v0.1.2가 기록한 "8회 invocation/16회 테스트"는 그 시점의 다른(임시 코드 기반) 절차에서 나온 수치이므로 HISTORICAL로 구분해 보존한다. **(4) DoD 개수 표기 정정**: `acceptance.md` §B의 실제 체크박스는 14개다(15개는 논리 AC 개수 — 015가 a/b 하위 시나리오를 가진 하나의 논리 AC이므로 DoD의 "AC-001~015(a/b 포함) 전체 PASS" 1줄이 그 15개를 대표한다). "DoD 15개"라는 progress.md의 표현을 정정한다.

> **개정 v0.1.7 (외부 재검토 4차 대응 — v0.1.6 서술 정확도 정정, 코드 변경 없음)**: v0.1.6의 수정 자체(10초 대기)는 유지하되, 그 설명에서 4가지를 정정했다 — 상세는 `progress.md` §E.2c(스케줄링/timeout 정정) 및 아래 이 문서 본문(1)~(2). **이 문서에서 직접 정정한 것**은 없음(이 AC 파일의 기존 서술 자체는 이 4개 지적과 충돌하지 않았다) — 다만 v0.1.6 항목 (4)/(6)이 "계측 1회 확인"과 "비계측 5+2회 재검증"을 이미 분리해 서술하고 있었음을 재확인했고, `progress.md`/`spec.md`/`CHANGELOG.md`의 동일 주제 서술을 이 문서와 일치하도록 그쪽에서 정정했다. 구현 변경이 없으므로 전체 E2E 5+2회는 재실행하지 않았다 — `pnpm lint`/`pnpm run format:check`는 재실행해 재확인했고(v0.1.6과 동일한 exit code, 신규 위반 0건), `pnpm run build`는 코드 무변경이라 재실행하지 않고 v0.1.6 결과(exit 0)를 재사용했다(`progress.md` §E.2d).
>
> **개정 v0.1.6 (외부 재검토 3차 대응 — 계측 기반 원인 확정 + auth.spec.ts 거짓 통과 정정 + 근본 수정, HEAD `49f986a` 대상)**: PR #9에 대한 외부 재검토가 v0.1.5의 원인 서술이 "실측"이 아니라 "정황 정합"에 머물러 있다고 지적해, 실제 `pnpm test:e2e` 진입점에서 `page.on("response", ...)` 계측(상대 타임스탬프+HTTP 상태+오류 코드, 비밀·쿠키·전체 응답은 기록하지 않음)으로 원인을 직접 재측정했다. **(1) 원인 확정(계측)**: 계측 1회차(수정 전)에서 setup-A(t=0)/setup-B(t=+542ms)/auth-success(t=+2280ms, 200)/auth-reject(t=+2799ms, **429** "Too many requests")/case-flow(t=+3297ms, **429**, retry #1로 31.75초 뒤 200) 순으로 관측 — case-flow의 로그인이 문자 그대로 5번째 `/sign-in/email` 요청이며 Better Auth 기본 rate limit(10초 창/최대 3회)을 초과함을 직접 확인했다(추정이 아니라 관측). **(2) [HARD] auth.spec.ts 거짓 통과 확정**: 같은 계측에서 `auth.spec.ts`의 "미등록 이메일 거부" 테스트가 **HTTP 429**로 응답받고도 PASS 처리되고 있었음을 발견 — `login-form.tsx`가 원인(코드)에 관계없이 `signInError.message`를 그대로 `login-error`에 표시하고, 그 테스트의 단언(`login-error` 가시성 + `/login` 유지)은 오류 원인을 구분하지 않기 때문이다. 즉 이 테스트는 SPEC 도입 이후 "미등록 이메일이 인증 로직에 의해 거부됨"이 아니라 "네 번째 sign-in 요청이 rate-limit에 걸림"을 우연히 검증하고 있었다 — REQ-E2EAUTH-011(`auth.spec.ts` 무변경)은 소스 레벨에서는 계속 참이지만, 그 테스트가 검증하는 **실질적 의미**가 이 SPEC의 부작용으로 바뀌어 있었다. **(3) 근본 수정**: `case-flow.spec.ts`/`auth.spec.ts`/`playwright.config.ts`의 `workers`/`retries`/`webServer`/Better Auth rate-limit 설정을 모두 무변경으로 유지한 채, 유일한 수정 가능 파일인 `e2e/auth.setup.ts`의 TESTER_B 로그인 완료 직후 10초 대기를 추가해 setup의 2건이 뒤이은 요청들과 다른 rate-limit 창에 들도록 했다 — 무근거 sleep이 아니라 관측된 10초 창 길이에서 직접 역산한 값이며, 후보로 검토한 (a) `case-flow.spec.ts` 수정 (b) `retries`/rate-limit 설정 조정 (c) Better Auth 완화는 모두 이 SPEC의 명시적 제약을 위반해 채택하지 않았다. **(4) 수정 후 재계측**: setup-A/setup-B/auth-success(200)/**auth-reject(t=+801ms, 401 `INVALID_EMAIL_OR_PASSWORD`, 진짜 자격증명 거부)**/case-flow(200, 1차 시도 통과, retry 없음) — false-pass가 해소되고 case-flow flakiness도 부수적으로 해소됨을 직접 확인했다. **(5) 실행 횟수 정정**: v0.1.5가 기록한 대상 2개 파일 "7/7"은 과잉 집계였다 — "독립 실행 7회(전체 스위트 5+필터 2)"와 "그 파일이 등장하는 실행 횟수"를 혼동했다. 올바른 분모는 setup(TESTER_A/B) 각 7회, 대상 2개 파일 각 6회(전체 5회 + 자신을 포함하는 필터 1회), `case-flow.spec.ts` 5회(필터에는 아예 등장하지 않음)다 — 상세 계산은 `progress.md` § case-flow 회귀 재조사(v0.1.6). **(6) 재검증**: 최종 코드로 전체 스위트 5회 연속 + `--spec` 필터 2회(총 7회, 전량 신규 실행 — 이전 버전 로그 재사용 없음)를 재실행해 setup A/B·대상 2개 파일·`case-flow.spec.ts`·auth.spec.ts 포함 전 항목이 매회 exit 0 + 1차 시도 통과(재시도 0건)임을 확인했다 — 상세: `progress.md` §E.2 (v0.1.6 재검증).
>
