...
> **개정 v0.1.7 (외부 재검토 4차 대응 — v0.1.6 서술 정확도 정정, 코드 변경 없음)**: v0.1.6의 수정 자체(10초 대기)는 유지하되, 그 설명에서 4가지를 정정했다 — 상세는 `progress.md` §E.2c(스케줄링/timeout 정정) 및 아래 이 문서 본문(1)~(2). **이 문서에서 직접 정정한 것**은 없음(이 AC 파일의 기존 서술 자체는 이 4개 지적과 충돌하지 않았다) — 다만 v0.1.6 항목 (4)/(6)이 "계측 1회 확인"과 "비계측 5+2회 재검증"을 이미 분리해 서술하고 있었음을 재확인했고, `progress.md`/`spec.md`/`CHANGELOG.md`의 동일 주제 서술을 이 문서와 일치하도록 그쪽에서 정정했다. 구현 변경이 없으므로 전체 E2E 5+2회는 재실행하지 않았다(lint/format/build만 재확인 — `progress.md` §E.2d).
>
