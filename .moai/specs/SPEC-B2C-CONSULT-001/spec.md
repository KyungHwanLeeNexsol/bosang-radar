...
- **REQ-B2CCONSULT-003** (When): 사용자가 02 화면의 4개 상담 CTA(상단 탑바/후유장해 섹션/하단 카카오톡/하단 전화) 중 하나를 클릭할 때, 시스템은 `/consult` 라우트로 실제 네비게이션을 수행하며 각 CTA에 대응하는 초기 채널 값(`kakao`/`kakao`/`kakao`/`phone`, 후유장해 섹션 CTA는 채널 미지정)을 URL 쿼리 파라미터(`?channel=...`)로 전달한다.
...
- **REQ-B2CCONSULT-005** (Where): `ENABLE_CONSULT_FLOW` 환경 변수(기본값 falsy)가 `"true"`가 아닐 때, 시스템은 02의 4개 상담 CTA를 현재의 `aria-disabled` "준비 중" stub 동작으로 **독립적으로** 유지한다 — 02 자체의 `shouldRenderDiagnosis` 게이트 상태와 무관하게 이 플래그 단독으로 CTA 활성화 여부를 결정한다.

### 3.3 02→03 핸드오프 · draft (Event-driven)

- **REQ-B2CCONSULT-006**: 시스템은 상담 폼의 임시 입력 상태(채널·이름·연락처 원문·연락 희망 시간·마케팅 동의 체크 여부·idempotencyKey)를 `sessionStorage`(`lib/consult/draft.ts`, 프로젝트 네임스페이스 전용 키)에 저장하되, 필수 동의 두 항목의 체크 상태는 저장하지 않는다(새로고침 시 항상 재확인 필요). draft 데이터가 손상되었거나 파싱 실패해도 오류 상태로 전환하지 않고 조용히 빈 draft로 폴백한다.
...
- **REQ-B2CCONSULT-017** (Unwanted): 시스템은 클라이언트가 제출한 `consultationId`/`createdAt`/`updatedAt`/`applicationStatus`/`consentVersion` 값을 서버 저장 시 신뢰해서는 안 된다 — 이 값들은 항상 서버가 자체 생성·스탬프한다.
...
- **REQ-B2CCONSULT-020**: 시스템은 기술적 멱등성(같은 버튼 재클릭·네트워크 재시도)을 `idempotencyKey` 컬럼의 DB UNIQUE 제약으로, 비즈니스 중복(이미 접수된 신청)을 `resultId`+정규화 연락처 복합 UNIQUE 인덱스로 각각 분리해 판정하며, 두 판정 모두 트랜잭션 내 원자적 DB 제약을 근거로 삼는다 — 클라이언트의 사전 체크는 참고용일 뿐 권위 있는 판정으로 취급하지 않는다.
- **REQ-B2CCONSULT-021**: 시스템은 동시에 도착한 동일 입력(같은 `idempotencyKey`)의 요청이 여러 건이어도 정확히 1개의 상담 신청 레코드만 생성되도록 보장한다.
...
- **REQ-B2CCONSULT-023** (When): 서버 응답이 `"duplicate"`일 때, 시스템은 마스킹된 연락처·접수일·처리 상태 라벨만 표시하며 기존 신청의 전체 페이로드나 내부 식별자를 노출하지 않는다.
...
- **REQ-B2CCONSULT-025**: 시스템은 `pnpm visual:verify`의 기존 15화면(01 계열 10 + 02 계열 5) 정의·허용 오차·02의 승인된 시각 debt 4건을 수정하지 않으며, 이 SPEC이 추가하는 9화면은 기존 배열에 추가하는 방식으로만 확장한다. 상담 신청 성공 시 `clearDiagnosisHandoff()`를 호출해 01→02 핸드오프를 정리하며, `DIAGNOSIS_ENGINE_READY`를 이 SPEC의 코드 어디에서도 `true`로 전환하지 않는다.
...
- Rate limiting의 구체 알고리즘·저장소(예: Redis 기반 슬라이딩 윈도)는 이 SPEC에서 확정하지 않는다 — 경량 카운터 방식의 존재만 계약하고 구현 세부는 run-phase로 위임한다.
