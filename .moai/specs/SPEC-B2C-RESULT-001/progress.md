...
- **사용자 결정 — 잔여 pixel 편차 2건 추가 승인 debt로 확정(2026-09-24)**: 위 3차 조사 결과를 사용자에게 다시 보고한 뒤(AskUserQuestion), 사용자는 "이 정도면 충분하다, 추가 승인 부채로 돌려달라"를 선택했다. 이에 따라 기존 2건에 아래 2건을 추가해 **승인된 debt를 정확히 4건**으로 확정한다:
  - ③ **집계 배너(`ResultAggregateBanner`) height 편차**(design 대비 42~48px 작음, 5화면 공통) — 카드/통계박스 경계는 픽셀 스캔으로 확인했으나 제목·부제·그리드 사이 여백의 정확한 원인은 특정하지 못한 채 사용자가 추가 조사 없이 현재 상태로 승인.
  - ④ **Mobile 입력 요약 카드 잔여 height 편차**(design 대비 55~57px 작음, top 8~9px 포함, M02/M02-B/M02-C/M02-D 4화면) — D3(3차)에서 이미 원인 대부분(구분선 누락, 편집 버튼 배치)을 해소해 원래 112~114px에서 절반 이상 줄었으나, 남은 차이는 사용자가 추가 조사 없이 현재 상태로 승인.
  - 이 두 건은 기존 ①·②(fixture 7개 유지, priorityChecklist 카드형 유지)와 성격이 다르다 — ①·②는 **의도적 제품 결정**(디자인 예시와 다르게 가기로 한 선택)이고, ③·④는 **원인 미확정 상태에서 추가 투자 대비 효용을 판단해 조사를 중단하기로 한 승인**이다. 이 구분을 progress.md에 명시해 향후 세션이 혼동하지 않도록 한다.
