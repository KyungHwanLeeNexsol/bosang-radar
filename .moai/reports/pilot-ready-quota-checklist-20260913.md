...
## RPD 운영 제한과 하이브리드 후속 결정

최초 확인 당시 병목은 `gemini-3.6-flash`의 20 RPD였으며 모든 사건이 Research 모델을
1회 사용하므로 운영 권장치를 하루 15~18건 이하로 정했다. 이후 2026-09-13 하이브리드
라우팅을 도입해 일반 사건은 Flash Lite로 시작하고 복합 쟁점 또는 Lite 품질 실패만
3.6 Flash로 승격하도록 변경했다. 따라서 사건 수의 일률적 상한은 더 이상 15~18건이
아니며, 실제 일일 수용량은 승격률과 재시도율에 따라 결정된다. 3.6 Flash 호출 자체는
여전히 재시도 여유를 포함해 하루 15회 이내를 보수적 운영 목표로 둔다. 상세 규칙은
`hybrid-research-routing-20260913.md`를 따른다.
