...
| 01-B 추가 질문 | `screenshots/01-B-questions.png` | `design/exports/01-B-추가-질문.png` | 일치 | 진행률 텍스트·라디오 테두리 일치. "다음"/"결과 보기" CTA에 BORA 퍼플(`variant="diagnosis"`) 적용 완료(오케스트레이터 후속 수정), "이전"은 ghost 유지 | "질문 N/3"·"건너뛰고 결과 보기"·"이전"/"다음" 문구 일치. 질문 문구 자체는 placeholder(design.md §15 각주, 잔여 위험으로 기존 기록됨) |
...
| 01-D 결과 없음 | `screenshots/01-D-result-none.png` | `design/exports/01-D-결과-없음.png` | 일치 | "내용을 수정할게요" 버튼에 BORA 퍼플 적용 완료(오케스트레이터 후속 수정) | 안내 문구 일치, mock 배지는 REQ-B2CDIAG-024 반영 신규 요소(결함 아님) |
| 01-E 분석 오류 | `screenshots/01-E-error.png` | `design/exports/01-E-분석-오류.png` | 일치 | 오류 아이콘 배경 destructive 톤 일치. "다시 시도" 버튼에 BORA 퍼플 적용 완료(오케스트레이터 후속 수정), "입력 내용으로 돌아가기"는 outline 유지 | 오류 문구·버튼 문구 일치 |
...
| M01-B 추가 질문 | `screenshots/M01-B-questions.png` | `design/exports/M01-B-추가-질문.png` | 레이아웃 일치. "다음" 버튼 BORA 퍼플 적용 완료(Desktop과 동일) |
...
| M-fix-3 (BORA 퍼플 CTA) | **PASS** | 진단 플로우 전체의 주요 CTA(보상 진단/동의하고 진단하기/확인/다음·결과 보기/내용을 수정할게요/다시 시도)가 전부 BORA 퍼플(`variant="diagnosis"`) 적용 확인. "이전"/"입력 내용으로 돌아가기"처럼 보조 동작인 버튼은 디자인 원본과 동일하게 outline/ghost 톤을 유지(의도된 위계 구분, 결함 아님) |
