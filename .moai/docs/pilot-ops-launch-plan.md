...
이메일은 이미 `.moai/docs/pilot-incident-runbook.md` §3의 파일럿 지원
연락처(이경환 담당, 1영업일 이내 1차 확인)로 확정된 동일 주소이며, 신규
이메일이 아니라 기존에 확정된 주소를 재사용한다.
...
- **임시 계정 대안 경로**: 대기하지 않고 임시(throwaway) 계정을 사용할
  경우, (i) 그 계정을 어떻게 발급하는지, (ii) 검증 후 §2.2와 동일한
  기준으로 어떻게 **정리**하는지, (iii) 그 계정과 그 계정이 생성한
  사건이 §3.2 성공지표 집계에서 명시적으로 **제외**됨을 모두 기록한다.
