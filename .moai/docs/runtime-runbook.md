...
### 알아두어야 할 것 — 실행이 끝나도 터미널이 반환되지 않을 수 있다 (Windows)

Windows 환경에서는 테스트가 전부 통과한 뒤에도 `pnpm test:e2e`가 즉시
종료하지 않고 터미널이 걸려 있는 채로 남을 수 있다. 이는 테스트 자체의
결함이 아니라, Playwright의 `webServer`(Next.js 서버) 프로세스를 정리
(teardown)하는 단계에서 pnpm의 셸 래핑과 Windows 프로세스 트리 종료 방식이
겹쳐 발생하는 알려진 문제다(자세한 재현 기록은
`.moai/specs/SPEC-RUNTIME-001/progress.md` §E.2 M5 절 참고).

터미널이 반환되지 않으면:

1. 출력에 `N passed`(모든 시나리오 통과) 요약이 이미 찍혀 있는지 먼저
   확인한다 — 찍혀 있다면 테스트는 이미 성공적으로 끝난 것이다.
2. 그 다음 `Ctrl+C`로 남은 프로세스를 종료한다.
3. CI나 스크립트에서 자동 실행할 때는 명령을 `timeout`으로 감싸 무기한
   대기를 방지하는 것을 권장한다.
