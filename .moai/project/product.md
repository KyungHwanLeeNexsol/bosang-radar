...
> 최종 수정: 2026-09-17 (디자인 피벗 반영 — B2B 손해사정사 워크스페이스 기획을
> 폐기하고, `design/MIGRATION-PLAN.md` 기준 B2C 보상 진단 퍼널로 제품 방향
> 전환. 이번 개정은 **문서 재작성만** 수행했다 — 기존 B2B 코드
> (`lib/pipeline/`, Better Auth, `/cases/*`)는 전혀 건드리지 않았고, 새
> B2C 화면(01/02/03)의 실제 구현도 아직 없다. 재사용/폐기 판단은 후속
> SPEC으로 유보. 이전 개정: 2026-09-16 SPEC-SIDEBAR-NAV-001 완료(15번째
> SPEC) 반영 — 사이드바 "전문가 피드백" 항목 아이콘을 `CornerDownRight`로
> 교체 + 활성 링크 `aria-label` 추가; 그 이전 개정 이력은 `git log
> .moai/project/product.md` 참고)
...
02 보상 진단 결과
  (4카테고리 담보 그리드: 실손의료비 · 정액담보 · 후유장해 · 특별보상)
        ↓
03 상담 신청
  (카톡 · 전화 선택 + 리드 폼 → 손해사정사 연결)
```

| # | 화면 | 역할 | 디자인 소스 |
|---|---|---|---|
| 01 | 질문 입력 | 사용자가 사고·질병·담보에 대해 검색창 한 줄로 질문 입력. 자주 찾는 사례 칩으로 빠른 진입 지원 | `design/claimradar-ui.pen` 노드 `l8dM0b`, `design/exports/01-보상-진단-질문-입력.png` |
| 02 | 보상 진단 결과 | **기준 디자인.** 4카테고리(실손의료비/정액담보/후유장해/특별보상) 담보 그리드. "해당 없음"도 숨기지 않고 사유와 함께 노출 | 노드 `A1oCfT`, `design/exports/02-보상-진단-결과.png` |
| 03 | 상담 신청 | 카톡/전화 선택 + 리드 폼(이름·연락처) → 손해사정사 연결 안내 | 노드 `Uli7t`, `design/exports/03-상담-신청-손해사정사-연결.png` |
...
전체 화면 구조·레이아웃·컴포넌트 토큰의 상세는 `design/MIGRATION-PLAN.md`가
SSOT다 — 이 문서는 요약만 담는다.
...
- **01 질문 입력 · 02 보상 진단 결과**: 비식별 유지. 이름·전화번호·주민번호
  등 PII 입력 필드가 없다. 사용자는 사고·질병 상황만 텍스트로 입력한다.
- **03 상담 신청**: 이번 피벗에서 새로 도입되는 **유일한 예외**다. 리드
  폼이 손해사정사 상담 연결을 위해 이름·연락처(전화 또는 카카오톡)를
  수집한다.
...
- **이 코드가 계속 존재하는 이유**: `lib/pipeline/`, `lib/auth/`(Better
  Auth), `app/cases/*`, `app/login/`, Turso DB 스키마 등은 모두 실제
  운영 중인 구현체다. 이번 문서 재작성은 이 코드를 삭제하거나 대체
  결정을 내리지 않는다 — 새 B2C 방향이 확정되기 전까지는 그대로
  유지된다.
...
`design/MIGRATION-PLAN.md` §6 "남은 작업" 기준. 아래는 **SPEC 후보**일 뿐,
이번 라운드에서 SPEC 문서를 생성하지 않는다:

- **01 상태 변형** — 입력 전 / 파싱 실패 시 되묻기 / 진단 중 로딩 / 결과
  없음 상태의 UI 정의 및 구현.
- **02 담보 데이터 확장** — 골절 외 암·뇌혈관·심장·디스크 등 케이스별
  담보 세트 데이터 설계.
- **02 결과 화면 실제 구현** — 디자인만 존재하는 4카테고리 담보 그리드를
  실제 라우트/컴포넌트로 구현 (담보 매칭 로직 방식은 미결정, 위 § 참고).
- **03 제출 완료 화면** — 신청 접수 확인 + 예상 연락 시점 UI.
