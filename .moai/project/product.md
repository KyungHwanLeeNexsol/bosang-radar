...
> 데이터만 렌더링한다. 참고: 이 개정 시점의 base는 `plan/SPEC-B2C-RESULT-001`
> 분기점(SPEC-B2C-DIAGNOSIS-001 병합 직후, `git branch --contains`
> 기준)이며, main에 이후 반영된 "SPEC-B2C-DIAGNOSIS-001 완료·3톤 상태명
> 통일" 문서 동기화 커밋은 이 개정에 포함되지 않는다 — 두 changeset은
> 병합 시점에 별도로 조정되어야 한다(잔여 위험). 이전 개정: 2026-09-18
