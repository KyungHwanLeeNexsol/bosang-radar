...
`design/MIGRATION-PLAN.md` §9 "남은 작업" 기준. 디자인은 01/02/03
전 상태(동의·추가 질문·로딩·에러·성공/중복/실패, Desktop+Mobile 24개)까지
확장 완료됐으므로, 아래는 그중 **아직 디자인·구현 모두 안 된 것**만
남긴다. 이번 라운드에서 SPEC 문서를 생성하지 않는다:

- **02 담보 데이터 확장** — 골절 외 암·뇌혈관·심장·디스크 등 케이스별
  담보 세트 데이터 설계 (현재는 골절 사례 1종만 디자인·문서화됨).
- **02/24개 화면 실제 구현** — 디자인만 존재하는 화면 전체를 실제
