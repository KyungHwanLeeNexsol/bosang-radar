...
> 최종 수정: 2026-09-17 (디자인 피벗 반영 — 이 문서는 더 이상 "제안 —
> Greenfield Plan"이 아니다. 아래 § 현재 구조(실측)는 실제 코드베이스를
> Glob/Read로 관찰한 결과다. B2C 새 화면(01/02/03)에 해당하는 코드는
> 아직 존재하지 않으며, 그 부분은 § 목표 구조(제안, 미구현)에 별도
> 표시했다. 이전 개정: 2026-08-25 SPEC-RUNTIME-001 후속)
...
- **§ 목표 구조 (제안, B2C 방향, 미구현)** — `design/MIGRATION-PLAN.md`가
  정의한 3화면 흐름을 구현할 때 어디에 위치할 가능성이 높은지의
...
`design/claimradar-ui.pen`이 새 B2C 3화면의 디자인 SSOT다(Pencil MCP
도구로만 열람 가능 — Read/Grep 대상 아님). `design/exports/*.png`는
Pencil에서 내보낸 화면별 정적 캡처로, 코드 구현 전 시각적 참고 자료다.
`design/MIGRATION-PLAN.md`가 디자인 언어·레이아웃·컴포넌트 토큰·폐기
이력을 담은 문서 SSOT다.
...
새 B2C 3화면(01 질문 입력 → 02 보상 진단 결과 → 03 상담 신청)에 대응하는
코드가 들어갈 가능성이 높은 위치:
